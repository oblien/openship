...
import { createSwarmDeployService, type SwarmDeployLogger } from "../apps/api/src/modules/deployments/swarm/deploy.service";
...
if (initial.stacks.some((stack) => stack.name === stackName)) {
  throw new Error(`Managed proof stack ${stackName} already exists; run scripts/swarm-lab.sh cleanup first.`);
}
...
const logger: SwarmDeployLogger = {
  ...
  log: (message) => { if (deployLogLines.length < 20) deployLogLines.push(message.trim()); },
  step: (phase, state, message) => process.stdout.write(`[swarm-managed] ${phase}:${state} ${message}\n`),
};
...
const deploy = createSwarmDeployService({
  ...
  createRevision: async (_stackId, _organizationId, data) => {
    const revision = { ...data, id: `revision-${++revisionNumber}`, stackId: binding.id, revision: revisionNumber } as SwarmStackRevision;
  ...
  syncProjections: async (_projectId, projections) => projections.map((projection, index) => {
  ...
  createServiceDeployments: async (rows) => { serviceDeployments.push(...rows); return [] as never; },
  now: () => new Date(),
});

const project = { id: projectId, organizationId } as Project;
const firstDeployment = { id: "deployment-managed-1", organizationId, createdAt: now } as Deployment;
const first = await deploy.deploy({ project, deployment: firstDeployment, environment: {}, logger });
if (first.state !== "ready" || binding.managementMode !== "managed") {
  throw new Error(`Expected first managed apply to converge and establish labels; state=${first.state} mode=${binding.managementMode}`);
}
if (serviceDeployments.length !== 2 || revisions.length !== 1 || first.runtimeRef.kind !== "swarm-stack") {
  throw new Error("Managed apply did not create the expected revision, stack reference, and two service deployment rows.");
}
...
const firstTaskIds = afterFirst.tasks
  ...
  .map((task) => task.id)
  .sort();
if (firstTaskIds.length !== 2) throw new Error("Managed fixture did not expose two current service tasks after apply.");

const secondDeployment = { id: "deployment-managed-2", organizationId, createdAt: now } as Deployment;
const second = await deploy.deploy({ project, deployment: secondDeployment, environment: {}, logger });
...
console.log(JSON.stringify({
  ...
  stack: stackName,
  deployments: [firstDeployment.id, secondDeployment.id],
  revisions: revisions.map((revision) => ({ id: revision.id, status: revision.applyStatus, digest: revision.renderedDigest })),
  serviceDeployments: serviceDeployments.map((row) => ({ serviceName: row.serviceName, runtimeRef: row.runtimeRef })),
  deployLogLines,
