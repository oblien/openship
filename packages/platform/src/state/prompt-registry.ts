 ...
 */
export const PROMPT_TIMEOUT_MS = DEFAULT_PROMPT_TIMEOUT_MS;
...
export class PromptRegistry {
  ...
  constructor(private readonly timeoutMs: number = PROMPT_TIMEOUT_MS) {
    if (!Number.isFinite(timeoutMs) || timeoutMs <= 0) throw new TypeError("Prompt timeout must be positive");
  }
  ...
  wait(key: string, onTimeout?: () => void): Promise<string> {
    if (this.pending.has(key)) return Promise.reject(new Error("A prompt is already pending for this session"));
  ...
  dispose(reason = "Prompt registry closed"): void {
    for (const key of this.pending.keys()) this.reject(key, reason);
  }
}
