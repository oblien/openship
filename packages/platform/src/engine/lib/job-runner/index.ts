...
/** Close an existing runner without creating one during shutdown. */
export async function shutdownJobRunner(deadlineMs = Infinity): Promise<void> {
  if (resolvingPromise) await resolvingPromise;
  if (instance) await instance.shutdown(deadlineMs);
  instance = null;
}
