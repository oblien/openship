...
export async function tunnelForward(
  ...
  const server = net.createServer((local) => {
    ...
    Promise.resolve().then(async () => {
      await opts.beforeConnect?.();
      if (closed || local.destroyed) return null;
      return tunnelConnect(serverId, remoteHost, remotePort);
    })
      .then((remote) => {
        if (!remote) return;
        // Shutdown may happen while SSH is opening the remote channel.
        if (closed || local.destroyed) { remote.destroy(); return; }
        local.pipe(remote);
        remote.pipe(local);
  ...
  const close = (): Promise<void> => {
    ...
    activeSockets.clear();
    return closePromise = new Promise(resolve => { server.close(() => resolve()); });
  };
  ...
  const localPort = await new Promise<number>((resolve, reject) => {
    ...
    const failed = (err: NodeJS.ErrnoException) => {
      if (err.code === "EADDRINUSE" && preferredPort !== 0 && !retried) {
