...
export async function startServerConnect(ctx: RequestContext, serverId: string) {
  ...
  return startServerDeviceFlow(flowKey(serverId), async (token) => {
    ...
    const current = await resolveExecutionAuthority(authority, `github-server:${serverId}`);
    await authorization.authorize(current, { resourceType: "server", resourceId: serverId, action: "write" });
...
export function cancelServerConnect(serverId: string): Promise<void> {
  return cancelDeviceFlow(flowKey(serverId));
}
...
export async function setServerToken(ctx: RequestContext, serverId: string, pat: string) {
  ...
  await cancelServerConnect(serverId);
  await repos.serverGithubAuth.upsert({
    serverId,
    organizationId: ctx.organizationId,
    mode: "token",
