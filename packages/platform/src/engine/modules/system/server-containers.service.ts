...
export function runContainerApply(
  ...
  const done = trackBackgroundWork((async (): Promise<ContainerApplyResult> => {
...
export async function scanOrgContainers(organizationId: string, beforeScan?: (server: Server) => Promise<void>): Promise<
...
export async function applyAllContainers(
  ...
  beforeApply?: (server: Server) => Promise<void>,
  ...
  // Authorize every target before probing or flagging accepted work. The same
  // callback runs again when a background worker reaches that target.
  for (const target of queue) await beforeApply?.(target.server);
