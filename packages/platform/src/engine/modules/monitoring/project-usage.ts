...
import { resolveLiveServiceState, type LiveServiceStatus } from "@repo/platform/engine/modules/services/live-state";
...
export async function openProjectUsageSampler(
  ...
  // The caller may close without reading a sample; still own initialization.
  ...
  return {
    ...
    close: async () => {
      await Promise.allSettled([capacityPromise, configPromise]);
