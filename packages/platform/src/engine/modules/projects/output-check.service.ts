...
import { resolveProjectRouteState } from "@repo/platform/engine/modules/domains/project-route.service";
import { auditStaticOutput, staticOutputTargets } from "@repo/platform/engine/modules/deployments/output-audit.service";
import { deploymentWorkload } from "@repo/platform/engine/modules/deployments/deployment-class";
...
export async function checkProjectOutput(
  ...
  return advisoryWork(runOutputProbe(project, deployment), [], OUTPUT_CHECK_BUDGET_MS);
}
