...
import { assertPublicUrl } from "@repo/platform/engine/lib/ssrf-guard";
...
import { mergeEnvVars } from "@repo/platform/engine/modules/projects/project-env.service";
import {
  createConnection,
  deleteConnection,
  internalModeAvailable,
} from "@repo/platform/engine/modules/projects/project-connection.service";
...
export async function getObjectStorage(
  ...
  ctx = await authorization.authorize(ctx, { resourceType: "project", resourceId: projectId, action: "read" });
  ...
  const candidates = await Promise.all(
    (await Promise.all(sources.map(async (p) =>
      await authorization.checkPermissionOnResource(ctx, { resourceType: "project", resourceId: p.id, action: "read" }) ? p : null,
    ))).filter((p): p is NonNullable<typeof p> => p !== null).map(async (p) => ({
      projectId: p.id,
      name: p.name,
