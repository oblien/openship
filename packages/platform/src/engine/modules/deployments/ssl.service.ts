 ...
 */
export async function getStatus(hostname: string, organizationId: string, assertAccess?: (domainId: string) => Promise<void>) {
  ...
  await assertAccess?.(domainRecord.id);
 ...
 * across tenants.
 */
async function renewOne(hostname: string, organizationId: string, assertAccess?: (domainId: string) => Promise<void>): Promise<SslRenewOutcome> {
  const domainRecord = await repos.domain.findByHostname(hostname);
  if (!domainRecord) throw new NotFoundError("Domain", hostname);
...
export async function renew(
  ...
  includeWww = false,
  assertAccess?: (domainId: string) => Promise<void>,
  ...
  const primary = await renewOne(hostname, organizationId, assertAccess);
