...
import type { ExecutionContext as RequestContext, ContextRole as RequestContextRole } from "../../../context";
import { checkPermissionOnResource } from "@repo/platform/engine/lib/authorization";
import { canUseGitHubRepo, resolveSourceAccess } from "@repo/platform/engine/modules/github/github-access";
import {
  AppError,
...
import { wildcardProjectGrantRejected, type TCreateTokenBody, type AuthorizeMcpClientInput } from "@repo/contracts";
import { captureExecutionAuthority, resolveExecutionAuthority } from "../../lib/execution-authority";

function assertCredentialManagement(ctx: RequestContext): void {
  if (ctx.tokenScope)
    throw new AppError("A scoped credential cannot create or change other credentials", 403, "CREDENTIAL_MANAGEMENT_DENIED");
}

function inScope(ctx: RequestContext, token: { organizationId: string | null }): boolean {
  return ctx.scopeMode !== "fixed" || token.organizationId === ctx.organizationId;
}

async function credentialExpiry(ctx: RequestContext): Promise<Date | null> {
  const current = ctx.principalKind
    ? await resolveExecutionAuthority(await captureExecutionAuthority(ctx), "credential-mint")
    : ctx;
  if (current.credential?.readOnly)
    throw new AppError("This access token is read-only", 403, "TOKEN_READ_ONLY");
  const ceiling = current.credential?.expiresAt;
  return ceiling == null ? null : new Date(ceiling);
}
...
async function mintContextFor(
  ...
  if (ctx.scopeMode === "fixed")
    throw new AppError("Select the target organization before authorizing this client", 403, "TOKEN_ORG_SCOPE");
...
export async function create(ctx: RequestContext, body: TCreateTokenBody) {
  assertCredentialManagement(ctx);
  ...
  if ("error" in intent) throw responseError(intent.error.body, intent.error.status);
  const wantScoped = intent.scoped;
  if (wantScoped) {
    const err = await validateGrants(ctx, grants);
    if (err) throw responseError(err.body, err.status);
  }

  const { token, tokenPrefix, tokenHash } = mintPatToken();
  let expiresAt = body.expiresInDays
    ? new Date(Date.now() + body.expiresInDays * 86_400_000)
    : null;
  // A short-lived embedding identity or token must not mint a credential that
  // silently outlives the identity that authorized it.
  const ceiling = await credentialExpiry(ctx);
  if (ceiling && (!expiresAt || expiresAt > ceiling)) expiresAt = ceiling;

  const row = await repos.personalAccessToken.createWithGrants({
    userId: ctx.userId,
...
export async function list(ctx: RequestContext) {
  ...
  return rows.filter(row => inScope(ctx, row)).map(serialize);
}

/** DELETE /api/tokens/:id — revoke one of the caller's own tokens. */
export async function revoke(ctx: RequestContext, id: string) {
  if (ctx.tokenScope && ctx.tokenScope.tokenId !== id) assertCredentialManagement(ctx);
  const row = await repos.personalAccessToken.findById(id);
  if (!row || row.userId !== ctx.userId || !inScope(ctx, row))
    throw responseError({ error: "Token not found", code: "NOT_FOUND" }, 404);
  const ok = await repos.personalAccessToken.revoke(id, ctx.userId);
  if (!ok) throw responseError({ error: "Token not found", code: "NOT_FOUND" }, 404);
  return { revoked: true };
}
 ...
 */
export async function authorizeMcpClient(ctx: RequestContext, body: AuthorizeMcpClientInput) {
  assertCredentialManagement(ctx);

  const clientId = body.clientId?.trim();
  if (!clientId) throw responseError({ error: "clientId required", code: "CLIENT_ID_REQUIRED" }, 400);
  ...
  const recorded = await repos.personalAccessToken.findOAuthBinding(ctx.userId, clientId, true);
  const existing = recorded && !recorded.revokedAt && (!recorded.expiresAt || recorded.expiresAt.getTime() > Date.now()) ? recorded : null;
  if (recorded && !inScope(ctx, recorded))
    throw responseError({ error: "Select the client's organization before changing its authorization", code: "TOKEN_ORG_SCOPE" }, 403);
  if (mode === "edit" && !existing) {
    throw responseError({ error: "MCP client is not connected", code: "MCP_CLIENT_NOT_CONNECTED" }, 404);
  }
  ...
  if (!mintCtx) {
    throw responseError({ error: "You are not a member of that organization", code: "ORG_NOT_A_MEMBER" }, 403);
  }
  ...
  if (grantErr) throw responseError(grantErr.body, grantErr.status);
  ...
  const binding = await repos.personalAccessToken.upsertOAuthBindingWithGrants({
    ...
    readOnly: body.readOnly ?? false,
    scoped,
    expiresAt: await credentialExpiry(ctx),
  ...
  audit.recordAsync({ ...operationAuditContext(ctx), organizationId }, {
  ...
  return { ok: true, scoped, readOnly: binding.readOnly };
}
...
function serializeBinding(
  b: { id: string; oauthClientId: string | null; name: string; organizationId: string | null; readOnly: boolean; scoped: boolean; createdAt: Date; lastUsedAt: Date | null; useCount: number; expiresAt?: Date | null },
  name: string,
...
export async function listMcpClients(ctx: RequestContext) {
  const bindings = (await repos.personalAccessToken.listOAuthBindings(ctx.userId)).filter(row => inScope(ctx, row));
 ...
 */
export async function disconnectMcpClient(ctx: RequestContext, client: string) {
  const clientId = client.trim();
  if (!clientId) throw responseError({ error: "clientId required", code: "CLIENT_ID_REQUIRED" }, 400);
  ...
  const binding = await repos.personalAccessToken.findOAuthBinding(ctx.userId, clientId, true);
  if (binding && !inScope(ctx, binding))
    throw responseError({ error: "MCP client is not connected", code: "MCP_CLIENT_NOT_CONNECTED" }, 404);
  const grantCount = binding ? (await repos.patGrant.listByToken(binding.id)).length : 0;
  if (ctx.tokenScope && ctx.tokenScope.tokenId !== binding?.id) assertCredentialManagement(ctx);
...
function responseError(body: { error: string; code?: string }, status: number): AppError {
  return new AppError(body.error, status, body.code);
}
