-- maxminddb.lua — VENDORED THIRD-PARTY. Do not edit; re-vendor instead.
--
-- Upstream: anjia0532/lua-resty-maxminddb v1.3.2 (opm). Loaded as
-- `openship.maxminddb` (see geo_country.lua). Verbatim except this header and ONE
-- marked change to the `ffi.load` call — search "OPENSHIP LOCAL MODIFICATION".
--
-- Why vendored rather than `opm get` at image build: `opm` needs perl, which the
-- openresty alpine base does not ship, and a build-time fetch would make the
-- image non-reproducible. Everything else in this directory is already shipped
-- by us (and base64-embedded into the JS bundle), so a pinned copy is the house
-- pattern — same reasoning as the vendored GeoLite2 DB in apps/api/assets/geoip.
--
-- Needs libmaxminddb.so at runtime (`apk add libmaxminddb`, in apps/edge/Dockerfile).
-- `ffi.load` below runs at REQUIRE time, so a missing .so throws on require —
-- geo_country.lua pcalls it for exactly that reason.
--
-- Re-vendor with:
--   curl -fsSL https://opm.openresty.org/api/pkg/tarball/anjia0532/lua-resty-maxminddb-<v>.opm.tar.gz \
--     | tar xz -O '*/lib/resty/maxminddb.lua' > packages/adapters/src/infra/lua/maxminddb.lua
--   # then re-apply this header and run `bun run embed:lua`
--[[
...
--
-- ── OPENSHIP LOCAL MODIFICATION (the only deviation from upstream) ───────────
-- Upstream is `ffi.load('libmaxminddb')`, which resolves the LINKER name
-- `libmaxminddb.so` — and that unversioned symlink ships in the -dev package, not
-- the runtime one. Alpine's `libmaxminddb` and Debian's `libmaxminddb0` both give
-- only `libmaxminddb.so.0`, so the bare name fails on a normal runtime install and
-- geo goes silently dark (geo_country pcalls this require). Try the soname too so
-- the runtime package alone is enough on every distro.
local maxm
do
    local candidates = { 'libmaxminddb', 'libmaxminddb.so.0', 'libmaxminddb.so' }
    local last_err
    for i = 1, #candidates do
        local ok, lib = pcall(ffi.load, candidates[i])
        if ok then maxm = lib; break end
        last_err = lib
    end
    if not maxm then
        error('libmaxminddb not loadable (tried ' .. table.concat(candidates, ', ') ..
              '): ' .. tostring(last_err), 2)
    end
end
