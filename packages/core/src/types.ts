...
export type ComposeAdvanced = {
  ...
  /**
   * Container hardening (#749) is the intersection below: `readOnly`, `capDrop`,
   * `securityOpt`, `tmpfs` and `user`, spread in FLAT beside the keys above so
   * they store, diff and round-trip exactly like `networkMode`.
   *
   * Their shape, what each accepts, why these five and not `cap_add` /
   * `privileged` / `devices` / `sysctls`, and why `read_only: false` is never
   * stored all live in ./compose-hardening.ts, the one authority the parser, the
   * CLI sync mapper and the Docker runtime each import. Declared there rather
   * than inline here for the same reason `compose-namespace.ts` exists: a second
   * spelling of the rules is how a stored control stops matching the container
