...
Under **Advanced** you can add a **Jump Host** (e.g. `user@bastion.example.com`), a **ProxyCommand**, and
**Extra SSH Arguments** (e.g. `-o StrictHostKeyChecking=no`). For Cloudflare Access, enter only the command body,
for example `"C:\\Program Files (x86)\\cloudflared\\cloudflared.exe" access ssh --hostname %h` — do not include
`ProxyCommand=`. ProxyCommand connections use the system OpenSSH client, so choose **Agent** or provide an
unencrypted key; OpenSSH runs non-interactively and cannot prompt for a password or key passphrase.
