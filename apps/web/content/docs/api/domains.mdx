...
| `POST /api/domains/check` | `domain:read` | Live-check whether those records already resolve — still creates nothing. |
...
## Check DNS without adding

Same records as preview, but Openship actually looks them up. Use this to confirm the registrar
published the values before you attach the domain.
...
```bash
curl -X POST https://your-host/api/domains/check \
  -H "Authorization: Bearer $OPENSHIP_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"hostname":"app.example.com","projectId":"proj_123"}'
```

```bash
openship domain check app.example.com -p proj_123
```

Each record comes back with `status` (`ok` / `missing` / `mismatch`) and the values currently observed.
