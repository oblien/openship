...
`domain.expiring`, `monitor.down`, `monitor.recovered`) that subscriptions and defaults key off. They live
in code, not the database, so `GET /api/notifications/categories` is a static registry — each entry has an
`id`, `label`, `description`, and `defaultEnabled`. The `monitor.*` pair comes from
[uptime monitors](/docs/api/monitors) and is on by default.
