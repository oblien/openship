...
You register **channels** (where alerts go — email, webhook, Slack, Discord, Microsoft Teams, Telegram, or the in-app bell), toggle
...
<TypeTable
  type={{
    ...
    "telegram.token": {
      type: "string",
      description: 'A Telegram bot token from @BotFather. Required for kind "telegram".',
    },
    "telegram.chatId": {
      type: "string",
      description: 'A numeric Telegram chat ID (user, group, or channel). Required for kind "telegram".',
    },
  }}
/>
...
<Callout title="Getting a Telegram bot token" type="info">
  Chat with [@BotFather](https://t.me/botfather) on Telegram to create a new bot and get its
  token (a string like `1234567890:ABCdef...`). To find your chat ID, message
  [@userinfobot](https://t.me/userinfobot) and it will reply with your numeric ID. For group or
  channel IDs, add the bot to the group and check the `getUpdates` response. The token is stored
  encrypted and comes back as `tokenConfigured` in responses, never in full.
</Callout>
...
<Callout title="400 — invalid channel or missing fields" type="warn">
  The `kind` isn't one of `email`/`webhook`/`in_app`/`slack`/`discord`/`msteams`/`telegram`, `label` is
