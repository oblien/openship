...
<Callout title="Managing the edge itself" type="info">
The commands above read one project. To manage the reverse proxy those requests pass through — a traffic
overview and per-domain analytics, route rules, registering domains on a port, or taking over a proxy
that's already on `:80/:443` — see [The edge](/docs/guides/the-edge) guide (or the
[`openship edge`](/docs/cli/edge) command reference).
</Callout>
