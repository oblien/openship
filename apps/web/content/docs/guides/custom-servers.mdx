...
- **Connection** — choose **Direct SSH** or **Cloudflare Access** for an existing Access SSH application.
...
For Cloudflare Access, enter the application's hostname (`ssh.example.com`) and your server's SSH
credentials. Complete the Cloudflare login on the same machine and under the same OS user that runs
OpenShip. Windows desktop includes the Cloudflare client. For a self-hosted instance or macOS/Linux,
install `cloudflared` on the machine running the API, or set `OPENSHIP_CLOUDFLARED_PATH` to its absolute
path. A containerized API needs the client and its authentication inside the container. The server's
SSH port stays behind the Cloudflare Tunnel; you do not need to expose port 22 publicly for this route.

The CLI uses the same connection setting:

```bash
openship server add --name Production --host ssh.example.com --ssh-transport cloudflare --auth-method agent
```
