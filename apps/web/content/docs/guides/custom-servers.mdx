...
There's an **Advanced** section too, for a **Jump Host** (a bastion server you must hop through first), a
**ProxyCommand**, and **Extra SSH Arguments**. For a Cloudflare Access server, enter the command body only,
for example `"C:\\Program Files (x86)\\cloudflared\\cloudflared.exe" access ssh --hostname %h` — omit
`ProxyCommand=`. ProxyCommand uses the system OpenSSH client; use **Agent** or an unencrypted key because the
connection is non-interactive.
