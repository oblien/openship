...
| `--proxy-command <command>` | OpenSSH ProxyCommand command body, e.g. `cloudflared access ssh --hostname %h`. Do not include `ProxyCommand=`. | — |
...
| `--ssh-proxy-command <command>` | OpenSSH ProxyCommand command body. Do not include `ProxyCommand=`. | — |
