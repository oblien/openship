...
Vite app, or the deploy "succeeds" but serves the wrong files.
</Callout>
...
If your project is a static site, a leftover start command is harmless — it's ignored, the deploy
continues, and the files are served from the edge.
</Callout>
...
## The app needs a bigger machine than this one

<Callout type="error">
`Pre-deploy checks failed: Host capacity: PostHog needs 8 GB of RAM (this machine has 2 GB) and
4 vCPU (this machine has 2 vCPU). Install it on a bigger machine, or pick a different destination.`
</Callout>

**What it means** — a few catalog apps are genuinely heavy (a nine-container analytics stack, a
Postgres fork that wants 8 GB) and declare a host floor. Openship reads what your destination's
Docker daemon reports and compares. This is the *first* install of that app, so it stops rather than
letting you watch nine containers get OOM-killed one by one.
...
<Step>
**Pick a bigger destination.** Reopen the app's install wizard — it states the floor next to the
destination picker and flags a server that's short *before* you install anything.
</Step>

<Step>
**Or resize the server** you already chose, then deploy again. The reading is cached briefly, so give
it a minute after a resize.
</Step>

</Steps>

<Callout type="info" title="It only ever stops a first install">
Once an app has deployed successfully, a shortfall is a warning, never a refusal — otherwise an app
running on a box that turned out to be undersized could never be redeployed. Likewise, a machine
Openship can't measure is never treated as too small, and Openship Cloud is sized from its tier, not
from host hardware. See [Host requirements](/docs/reference/app-catalog).
</Callout>
...
<Callout type="warn" title="“This operation targets the HOST machine, but no host channel is configured”">
Not a build failure at all: this containerized install can't reach the machine it runs on, so the one
step that needs the host (a `:80`/`:443` takeover, the mail engine, a catalog app whose config file is
written on the host) refuses rather than run against the wrong filesystem. Ordinary deploys are
unaffected. The repair differs by install shape —
[Host control channel](/docs/troubleshooting/host-channel).
</Callout>
