...
<Callout title="Not this page: the box Openship itself runs on" type="info">
A containerized install reaches **its own host** over a separate SSH channel that Openship provisions
for itself — no credentials you typed, and nothing on the Servers screens. If your symptom is
*"This operation targets the HOST machine, but no host channel is configured"*, a server that reads
**Offline** while its containers are plainly running, or a `:80`/`:443` takeover that won't start, go to
[Host control channel](/docs/troubleshooting/host-channel) instead.
</Callout>
...
### A ProxyCommand, jump host, or extra SSH args seem ignored

<Callout type="error">
You set a **ProxyCommand**, **Jump Host** (bastion), or **Extra SSH Arguments** under **Advanced**, but the
connection behaves as if they weren't there.
</Callout>

**What it means.** These options are applied by the system OpenSSH client. A **ProxyCommand** always selects
that client, even when the server uses pasted or host-path key auth. Password auth cannot be supplied to this
non-interactive path; an encrypted key passphrase cannot be prompted either.
...
<Step>
Switch the server's **Authentication** to **Agent**, or use a key without a passphrase, so the ProxyCommand,
jump host, and extra args take effect without an interactive prompt.
</Step>
...
<Step>
For Cloudflare Access, put only the command body in **ProxyCommand**, such as
`cloudflared access ssh --hostname %h` or `"C:\\Program Files (x86)\\cloudflared\\cloudflared.exe" access ssh --hostname %h`.
Do not include the `ProxyCommand=` prefix. Alternatively, configure the bastion/proxy once in your `~/.ssh/config`
and use **Agent**; that path reads your SSH config the same way your terminal does.
</Step>
