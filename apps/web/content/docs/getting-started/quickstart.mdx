...
Already have a package manager? `npm i -g openship` works too (or `pnpm`, `yarn`, `bun`) — that
route needs **Node 22+**, since it runs on your Node rather than bringing its own.
