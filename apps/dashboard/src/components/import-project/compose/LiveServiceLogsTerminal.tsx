...
export const LiveServiceLogsTerminal: React.FC<LiveServiceLogsTerminalProps> = ({
  ...
  const logStream = useLogStream({
    ...
    callbacks: {
      onLog: (message, rawText, rawBytes) => {
        ...
        // Docker emits complete lines; raw cloud streams already carry their
        // own control characters. Preserve bytes and separate plain log lines.
        if (message.data && rawBytes) terminalRef.current?.write(rawBytes);
        else terminalRef.current?.write(`${(rawText ?? message.message ?? "").replace(/\r?\n$/, "")}\r\n`);
      },
      onError: (message) => {
    ...
    onError: (error) => {
      if (stoppingRef.current) return;
      terminalRef.current?.write(`\x1b[1;31m[${error.message}]\x1b[0m\r\n`);
  ...
  const connect = useCallback(async () => {
    ...
    const query = new URLSearchParams({ tail: "100", deploymentId });
    await logStream.connect(`${endpoints.services.logsStream(projectId, serviceId)}?${query}`);
  }, [logStream, projectId, serviceId, deploymentId]);
