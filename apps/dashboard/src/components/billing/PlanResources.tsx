...
import { ChevronDown, Clock3, Cpu, FolderOpen, Globe2, Layers3 } from "lucide-react";
...
export function PlanResources({ plan, interval = "monthly" }: { plan: ApiPlan; interval?: "monthly" | "annual" }) {
  ...
  const facts = [
    { Icon: FolderOpen, label: copy.projects, value: count(plan.limits.maxProjects) },
    { Icon: Layers3, label: copy.apps, value: count(plan.limits.runningServices) },
    { Icon: Clock3, label: copy.buildTime, value: plan.limits.buildMinutesPerMonth === null ? copy.unlimited : interpolate(copy.buildMinutes, { amount: count(plan.limits.buildMinutesPerMonth) }) },
    { Icon: Cpu, label: copy.machine, value: spec ? `${formatCpuCores(spec.cpuCores)} · ${formatMemoryMb(spec.memoryMb)}` : copy.unlimited },
    ...(plan.edge ? [{ Icon: Globe2, label: t.billing.resourceOverview.bandwidth, value: plan.edge.bandwidthGb === null ? copy.unlimited : interpolate(t.billing.resourceOverview.bandwidthPerMonth, { amount: count(plan.edge.bandwidthGb) }) }] : []),
  ];
  return (
    <div className="border-t border-border/40 py-4">
      <dl className="space-y-4">
        {facts.map(({ Icon, label, value }) => <div key={label} className="flex flex-wrap items-start justify-between gap-x-3 gap-y-1">
          <dt className="flex items-center gap-2 text-xs text-muted-foreground"><Icon className="size-3.5 shrink-0" aria-hidden="true" />{label}</dt>
      ...
      <details className="group mt-5 rounded-lg bg-muted/35 p-3">
        <summary className="flex cursor-pointer list-none items-center justify-between gap-2 text-xs font-medium text-foreground [&::-webkit-details-marker]:hidden">
          {copy.usageIncluded}<ChevronDown className="size-3.5 shrink-0 transition-transform group-open:rotate-180" aria-hidden="true" />
        </summary>
