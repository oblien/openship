...
export function HealthBadge({ state }: { state: string }) {
  ...
  if (state === "ready" || state === "converged" || state === "clean") {
    return (
      <span className={`${common} bg-success/10 text-success`}>
  ...
  if (
    ...
    return (
      <span className={`${common} bg-danger-bg text-danger`}>
  ...
  if (state === "paused") {
    return (
      <span className={`${common} bg-warning/10 text-warning`}>
  ...
  return (
    <span className={`${common} bg-muted text-muted-foreground`}>
  ...
  if (nodes.length === 0) {
    return (
      <p className="py-6 text-center text-sm text-muted-foreground">
        No nodes were returned by this manager.
      </p>
    );
  }
  return (
          ...
          {nodes.map((node) => (
            <tr key={node.id}>
              ...
              <td className="px-4 py-3">
                <HealthBadge state={node.status.toLowerCase()} />
              </td>
...
export function SwarmTasksTable({
  tasks,
  onLogs,
  page,
  onPage,
  ...
  if (tasks.length === 0) {
    return (
      <p className="py-6 text-center text-sm text-muted-foreground">
        No current tasks were returned for this stack.
      </p>
    );
  }
  return (
    <div>
          ...
          <tbody className="divide-y divide-border/40">
            {tasks.map((task) => (
              <tr key={task.id}>
                ...
                <td className="px-4 py-3">
                  <span className={task.error ? "text-danger" : "text-foreground"}>
                    {task.currentState}
                  </span>
      ...
      {page && page.total > page.limit && (
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border/50 px-4 py-3 text-xs text-muted-foreground">
          <span>
            Showing {page.offset + 1}–{page.offset + tasks.length} of {page.total} manager task
            records
          </span>
          {onPage && (
            <div className="flex items-center gap-2">
              <button
                ...
                className="inline-flex items-center gap-1 rounded-lg border border-border px-2 py-1 font-medium text-foreground disabled:opacity-50"
              >
