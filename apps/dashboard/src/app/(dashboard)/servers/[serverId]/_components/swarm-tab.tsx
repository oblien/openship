...
import { Boxes, ChevronDown, ChevronUp, Eye, Loader2, Plus, RefreshCw, Server, ShieldCheck, TriangleAlert } from "lucide-react";
import { getApiErrorMessage, projectsApi, registriesApi, swarmApi, systemApi, type ContainerRegistry, type SwarmDiscoveryView, type SwarmNode, type SwarmStackDetail, type SwarmSummary } from "@/lib/api";
      ...
      <section className="rounded-2xl border border-primary/25 bg-primary/[0.04] p-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <div className="flex items-center gap-2"><h2 className="font-semibold text-foreground">Docker Swarm</h2><span className="rounded-full bg-warning/10 px-2 py-0.5 text-[11px] font-semibold text-warning">Experimental</span></div>
              <p className="mt-1 max-w-2xl text-sm leading-relaxed text-muted-foreground">Inspect existing stacks without changing them, or reserve a new stack namespace and configure its authoritative source before the first reviewed apply.</p>
            </div>
          </div>
      ...
      <section className="rounded-2xl border border-border/50 bg-card p-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 max-w-3xl text-sm leading-relaxed text-muted-foreground">Reserve an unused stack namespace on a manager, choose its source and registry, then dry-run the rendered configuration before OpenShip can apply anything.</p>
          </div>
          <button type="button" onClick={() => setCreatingProject((open) => !open)} className="inline-flex items-center gap-2 rounded-xl bg-primary px-3 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90">
            <Plus className="size-3.5" /> {creatingProject ? "Close setup" : "Create stack project"}
          </button>
        </div>
        {creatingProject && <NewSwarmStackProject
...
function NewSwarmStackProject({
  ...
  useEffect(() => {
    let cancelled = false;
    void registriesApi.list()
      .then((items) => {
        if (!cancelled) setRegistries(items);
      })
      .catch((cause) => {
        if (!cancelled) showToast(getApiErrorMessage(cause, "Unable to load container registries."), "error", "Docker Swarm");
      })
      .finally(() => {
    ...
    void systemApi.listServers()
      .then(async (servers) => {
        ...
        setManagerCandidates([
          ...
          ...summaries.filter((candidate): candidate is SwarmManagerCandidate => candidate !== null),
        ]);
      })
      .catch(() => {
  ...
  const selectedManager = managerCandidates.find((candidate) => candidate.serverId === selectedManagerId)
    ?? managerCandidates[0]!;
  const composePaths = repositoryPaths.split(",").map((path) => path.trim()).filter(Boolean);
  ...
  const stackNameError = !stackName.trim()
    ...
    : stackName.length > 63 || !STACK_NAME_PATTERN.test(stackName)
      ...
      : null;
  const repositoryError = sourceMode !== "repository"
    ? null
    : !repositoryOwner.trim() || !repositoryName.trim()
      ...
      : composePaths.length === 0 || composePaths.some((path) => !isRelativeRepositoryPath(path))
        ...
        : sourcePath.trim() && !isRelativeRepositoryPath(sourcePath)
          ...
          : null;
  const inlineError = sourceMode === "inline" && !inlineYaml.trim()
    ? "Paste the complete stack YAML."
    : null;
  const canSubmit = !projectNameError && !stackNameError && !repositoryError && !inlineError && !submitting;
  ...
  const createProject = async () => {
    ...
    try {
      ...
      showToast("Stack project created in read-only setup mode. Render and review it before the first apply.", "success", "Docker Swarm");
  ...
  return <div className="mt-5 rounded-xl border border-primary/20 bg-primary/[0.025] p-4">
    <div className="grid gap-4 lg:grid-cols-2">
      <div className="space-y-4">
        <div>
          <h3 className="font-medium text-foreground">1. Stack identity</h3>
          <p className="mt-1 text-sm text-muted-foreground">The namespace is checked on the selected manager before OpenShip records the binding.</p>
        </div>
        <label className="block text-sm text-muted-foreground">Project name
          <input value={projectName} maxLength={100} onChange={(event) => {
            const value = event.target.value;
            setProjectName(value);
            if (!stackNameTouched) setStackName(suggestedStackName(value));
          }} placeholder="Production web stack" className="mt-1.5 w-full rounded-xl border border-border bg-card px-3 py-2 text-sm text-foreground outline-none focus:border-primary" />
          {projectNameError && <span className="mt-1 block text-xs text-danger">{projectNameError}</span>}
        </label>
        <label className="block text-sm text-muted-foreground">Stack name
          <input value={stackName} maxLength={63} onChange={(event) => { setStackNameTouched(true); setStackName(event.target.value); }} placeholder="production-web" className="mt-1.5 w-full rounded-xl border border-border bg-card px-3 py-2 font-mono text-sm text-foreground outline-none focus:border-primary" />
          {stackNameError ? <span className="mt-1 block text-xs text-danger">{stackNameError}</span> : <span className="mt-1 block text-xs text-muted-foreground">This name is reserved without creating services, networks, volumes, configs, or secrets.</span>}
        </label>
        <fieldset>
          <legend className="text-sm text-muted-foreground">Manager preference</legend>
          <div className="mt-1.5 space-y-2">
            {managerCandidates.map((candidate) => <label key={candidate.serverId} className={`flex cursor-pointer items-start gap-3 rounded-xl border p-3 ${selectedManagerId === candidate.serverId ? "border-primary/40 bg-primary/[0.035]" : "border-border/50 bg-card"}`}>
              <input type="radio" name="swarm-manager" value={candidate.serverId} checked={selectedManagerId === candidate.serverId} onChange={() => setSelectedManagerId(candidate.serverId)} className="mt-0.5" />
              <span className="min-w-0"><span className="block text-sm font-medium text-foreground">{candidate.label}</span><span className="mt-0.5 block text-xs text-muted-foreground">Cluster fingerprint <code>{shortId(candidate.summary.manager.clusterId)}</code> · {candidate.summary.manager.managerAddress || candidate.summary.manager.nodeAddress || "active manager"}</span></span>
            </label>)}
            {loadingManagers && <p className="flex items-center gap-2 text-xs text-muted-foreground"><Loader2 className="size-3 animate-spin" />Checking other registered managers…</p>}
          </div>
        </fieldset>
      </div>
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Source is saved first, then rendered on the selected manager for a read-only comparison.</p>
        </div>
        <div className="flex rounded-xl border border-border/50 bg-card p-1">
          {([["inline", "Paste YAML"], ["repository", "Repository files"]] as const).map(([mode, label]) => <button key={mode} type="button" onClick={() => setSourceMode(mode)} className={`flex-1 rounded-lg px-3 py-2 text-sm font-medium ${sourceMode === mode ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}>{label}</button>)}
        </div>
        {sourceMode === "inline" ? <label className="block text-sm text-muted-foreground">Stack YAML
          <textarea value={inlineYaml} onChange={(event) => setInlineYaml(event.target.value)} spellCheck={false} placeholder={"services:\n  web:\n    image: nginx:alpine"} className="mt-1.5 min-h-48 w-full rounded-xl border border-border bg-card p-3 font-mono text-xs text-foreground outline-none focus:border-primary" />
          {inlineError && <span className="mt-1 block text-xs text-danger">{inlineError}</span>}
        </label> : <div className="grid gap-3 sm:grid-cols-2">
          <SwarmField label="GitHub owner" value={repositoryOwner} onChange={setRepositoryOwner} placeholder="acme" />
          <SwarmField label="GitHub repository" value={repositoryName} onChange={setRepositoryName} placeholder="production-stack" />
          <SwarmField label="Branch" value={repositoryBranch} onChange={setRepositoryBranch} placeholder="main" />
          <SwarmField label="Source subdirectory (optional)" value={sourcePath} onChange={setSourcePath} placeholder="deploy" />
          <div className="sm:col-span-2"><SwarmField label="Compose paths, in merge order" value={repositoryPaths} onChange={setRepositoryPaths} placeholder="compose.yaml, compose.production.yaml" /></div>
          {repositoryError && <p className="sm:col-span-2 text-xs text-danger">{repositoryError}</p>}
        </div>}
        <label className="block text-sm text-muted-foreground">Registry
          <select value={registryId} disabled={loadingRegistries} onChange={(event) => setRegistryId(event.target.value)} className="mt-1.5 w-full rounded-xl border border-border bg-card px-3 py-2 text-sm text-foreground outline-none focus:border-primary disabled:opacity-60">
            <option value="">No registry selected</option>
            {registries.map((registry) => <option key={registry.id} value={registry.id}>{registry.name} · {registry.registryUrl}</option>)}
          </select>
          <span className="mt-1 block text-xs text-muted-foreground">{loadingRegistries ? "Loading registries…" : "Prebuilt-image stacks can leave this unset; source-built services need a registry before apply."}</span>
        </label>
        <fieldset>
          <legend className="text-sm text-muted-foreground">Routing mode</legend>
          <label className="mt-1.5 flex items-start gap-3 rounded-xl border border-primary/40 bg-primary/[0.035] p-3"><input type="radio" checked readOnly className="mt-0.5" /><span><span className="block text-sm font-medium text-foreground">External controller</span><span className="mt-0.5 block text-xs text-muted-foreground">Router labels, domains, TLS, and ports remain outside OpenShip.</span></span></label>
          <p className="mt-2 text-xs text-muted-foreground">OpenShip Edge becomes selectable only after a reviewed source is claimed, so an unowned stack can never take over traffic by mistake.</p>
        </fieldset>
      </div>
    </div>
    {submissionError && <div role="alert" className="mt-4 rounded-xl border border-danger/25 bg-danger-bg/35 px-3 py-2 text-sm text-danger">{submissionError}{partialProjectId && <button type="button" onClick={() => router.push(`/projects/${partialProjectId}/overview`)} className="ml-2 font-medium underline underline-offset-2">Open saved project</button>}</div>}
    <div className="mt-5 flex flex-wrap items-center justify-end gap-3 border-t border-border/50 pt-4"><button type="button" disabled={submitting} onClick={onClose} className="rounded-xl px-3 py-2 text-sm font-medium text-muted-foreground hover:bg-muted disabled:opacity-60">Cancel</button><button type="button" disabled={!canSubmit} onClick={() => void createProject()} className="inline-flex items-center gap-2 rounded-xl bg-primary px-3 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 disabled:opacity-60">{submitting && <Loader2 className="size-3.5 animate-spin" />}{submitting ? "Reserving stack namespace…" : "Create read-only stack project"}</button></div>
  </div>;
}

function SwarmField({ label, value, onChange, placeholder }: { label: string; value: string; onChange: (value: string) => void; placeholder: string }) {
  return <label className="block text-sm text-muted-foreground">{label}<input value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} className="mt-1.5 w-full rounded-xl border border-border bg-card px-3 py-2 text-sm text-foreground outline-none focus:border-primary" /></label>;
}
