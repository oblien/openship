...
import { DnsRecordsTable, mergeCheckedRecords, type DnsTableRecord } from "./domains/DnsRecordsTable";
import { AddDomainModal, type AddDomainDraft } from "./domains/AddDomainModal";
...
function toEditablePublicEndpoint(endpoint: any): PublicEndpoint {
  return createPublicEndpoint({
    ...
    redirectStatus: typeof endpoint?.redirectStatus === "number" ? endpoint.redirectStatus : undefined,
  });
}
...
function matchPublicEndpoint(
  endpoints: PublicEndpoint[],
  summary: Pick<DomainSummaryItem, "id" | "domainId" | "hostname">,
  baseDomain: string,
): PublicEndpoint | undefined {
  const hostname = summary.hostname.toLowerCase();
  return endpoints.find((ep) =>
    (summary.domainId && ep.id === summary.domainId) ||
    ep.id === summary.id ||
    resolveProjectEndpointHostname(ep, baseDomain) === hostname,
  );
}
...
export const DomainSettings = () => {
  ...
  const handleSubmitDomains = async (draft: AddDomainDraft): Promise<boolean> => {
    const host = draft.hostname.trim().toLowerCase();
    if (!host) return false;
    const isCustom = draft.domainType === "custom";
    ...
    try {
      ...
      if (isCustom) {
        const result = await projectsApi.connectDomain(id, {
          ...
          externalIngress: draft.externalIngress,
        });
        if (!result.success) {
          showToast(
      ...
      const routeTarget = hasProjectServer
        ? { port: portValue }
        : { targetPath: draft.path.trim() || "/" };
      const nextEndpoints = [
        createPublicEndpoint({
  ...
  }, [pendingDomainAction, domainsData.isLoading, setPendingDomainAction]);
  ...
  const handleStartEditingDomain = (summary: DomainSummaryItem) => {
    setPublicEndpoints(draftPublicEndpoints);
    const matched = matchPublicEndpoint(draftPublicEndpoints, summary, baseDomain);
    setEditingDomainId(matched?.id ?? summary.id);
  };

  const handleCancelEditingDomain = () => {
    setPublicEndpoints(draftPublicEndpoints);
  ...
  // service route cards so both collapse the same way. The hostname itself is
  // the visit link. `onEditRoute` adds the per-service "Edit route" item.
  ...
  const editingSummary = editingDomainId
    ? domainSummaries.find((d) => d.id === editingDomainId)
    : undefined;
  const editingEndpoint = editingDomainId
    ? publicEndpoints.find((ep) => ep.id === editingDomainId)
      ?? (editingSummary ? matchPublicEndpoint(publicEndpoints, editingSummary, baseDomain) : undefined)
    : undefined;
  ...
  const addDomainAction = (
    <ActionButton
      label={t.projectSettings.domains.actions.addDomain}
      icon={Plus}
    ...
    <div className="space-y-5">
      {domainsData.isLoading ? (
        <div className="rounded-xl border border-border/50 bg-card">
          {[0, 1].map((i) => (
            <div key={i} className="flex items-center gap-3 border-b border-border/40 px-3 py-2.5 last:border-0">
              <div className="h-3.5 w-40 animate-pulse rounded bg-muted/60" />
              <div className="ms-auto h-5 w-16 animate-pulse rounded-full bg-muted/50" />
            </div>
          ))}
        </div>
      ) : null}
      ...
      <AddDomainModal
        ...
        hasServer={hasProjectServer}
        defaultPort={projectRuntimePort}
        onRequireCloud={freeNeedsCloud}
        onCopy={handleCopy}
        onAdd={handleSubmitDomains}
      />
      {!hasDomain && !domainsData.isLoading ? (
        <DomainList
          title={t.projectSettings.domains.setup.listTitle}
          toolbar={addDomainAction}
        >
          <div className="px-3 py-4">
            <p className="text-[13px] font-medium text-foreground">{t.projectSettings.domains.setup.emptyTitle}</p>
            <p className="mt-0.5 text-[12px] text-muted-foreground">{t.projectSettings.domains.setup.emptyBody}</p>
            <p className="mt-3 truncate font-mono text-[12px] text-muted-foreground">{currentUrl}</p>
          </div>
        </DomainList>
      ) : null}

      {hasDomain && hasProjectLevelRouting ? (
        <DomainList
          title={t.projectSettings.domains.setup.listTitle}
          toolbar={
            <>
        ...
        >
          {domainSummaries.map((domain) =>
            renderRouteCard(domain, {
              onEdit: () => handleStartEditingDomain(domain),
              onSetPrimary:
      ...
      {hasProjectLevelRouting && editingEndpoint ? (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4"
          onClick={handleCancelEditingDomain}
        >
          <div
          ...
          >
            <div className="px-5 pt-5">
              <h3 className="text-[15px] font-semibold text-foreground">{t.projectSettings.domains.edit.title}</h3>
            ...
            <div className="px-5 py-5">
              <PublicEndpointsCard
                ...
                hasServer={hasProjectServer}
                runtimePort={editingEndpoint.port || projectRuntimePort}
                onChange={(nextEndpoints) => {
                  const edited = nextEndpoints[0];
                  if (!edited) return;
                  setPublicEndpoints((prev) =>
                    prev.map((ep) => (ep.id === edited.id ? edited : ep)),
                  );
                }}
                allowAdd={false}
                hideHeader
            ...
            <div className="flex items-center justify-between gap-3 px-5 pb-5">
              <button
                ...
                className="inline-flex min-h-8 items-center rounded-lg border border-border px-3 text-[13px] font-medium text-foreground hover:bg-foreground/[0.04] disabled:opacity-50"
              >
              ...
              <button
                ...
                disabled={isSavingPublicEndpoints}
                className="inline-flex min-h-8 items-center gap-1.5 rounded-lg bg-foreground px-3.5 text-[13px] font-medium text-background hover:bg-foreground/90 disabled:cursor-not-allowed disabled:opacity-50"
              >
                {isSavingPublicEndpoints ? <Loader2 className="size-3.5 animate-spin" /> : null}
      ...
      {!hasProjectLevelRouting && (servicesLoading || services.length > 0) && (
        <div className="space-y-3">
          ...
          {servicesLoading ? (
            ...
            <DomainList title={t.projectSettings.domains.setup.listTitle}>
      ...
      {editingRouteService && editingRoute && routeDraft && (
        ...
        >
          ...
          >
            <div className="px-5 pt-5">
              <h3 className="text-[15px] font-semibold text-foreground">{t.projectSettings.domains.editRoute.title}</h3>
            ...
            <div className="flex items-center justify-between gap-3 px-5 pb-5">
              <button
                type="button"
                onClick={() => setEditingRouteServiceId(null)}
                className="inline-flex min-h-8 items-center rounded-lg border border-border px-3 text-[13px] font-medium text-foreground hover:bg-foreground/[0.04]"
              >
              ...
              <button
                ...
                className="inline-flex min-h-8 items-center gap-1.5 rounded-lg bg-foreground px-3.5 text-[13px] font-medium text-background hover:bg-foreground/90 disabled:cursor-not-allowed disabled:opacity-50"
              >
                {routeSaving ? <Loader2 className="size-3.5 animate-spin" /> : null}
...
function ActionButton({
  ...
  const className =
    "inline-flex min-h-8 items-center gap-1.5 rounded-lg bg-foreground/[0.06] px-2.5 py-1.5 text-[12px] font-medium text-foreground transition-colors hover:bg-foreground/[0.1] disabled:cursor-not-allowed disabled:opacity-50";
  ...
  return (
    <div className="rounded-xl border border-border/50 bg-card">
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border/40 px-3 py-2">
        ...
        {toolbar ? <div className="flex flex-wrap items-center gap-1.5">{toolbar}</div> : null}
      </div>
      <div>{children}</div>
    </div>
  );
...
function DomainOverviewCard({
  ...
  const [records, setRecords] = useState<DnsTableRecord[] | null>(null);
  const [recordsLoading, setRecordsLoading] = useState(false);
  const [checking, setChecking] = useState(false);
  const [checkHint, setCheckHint] = useState<string | null>(null);

  useEffect(() => {
    if (!canVerify || !domain.domainId) return;
    ...
    void domainsApi
      .records(domainId)
      .then((result) => {
        if (!cancelled) setRecords(result.data.records);
      })
      .catch(() => {
  ...
  }, [canVerify, domain.domainId]);

  const handleFinish = async () => {
    if (!onVerify || verifying || checking || recordsLoading) return;
    if (!records?.length) {
      onVerify();
      return;
    }
    ...
    try {
      ...
      const merged = mergeCheckedRecords(records, result.data.records);
      setRecords(merged);
      if (!result.data.allOk) {
        setCheckHint(d.setup.verifyWait);
        return;
      }
    } catch {
      // Check is best-effort; Verify still confirms on cloud.
    } finally {
      setChecking(false);
    }
    onVerify();
  ...
  return (
    <div className="border-b border-border/40 last:border-0 last:rounded-b-xl">
      <div className="flex items-center gap-3 px-3 py-2.5">
        <div className="min-w-0 flex-1">
          <div className="flex min-w-0 items-center gap-2">
            {domain.liveUrl ? (
              ...
              <span className="truncate text-[13px] font-medium text-foreground">{domain.hostname}</span>
            )}
            {domain.isPrimary ? (
        ...
        <StatusPill tone={domain.status.tone}>{domain.status.label}</StatusPill>
        {menuActions.length > 0 ? <DropdownMenu actions={menuActions} align="right" /> : null}
      </div>

      {portHint ? (
        <p className="flex items-start gap-1.5 px-3 pb-2 text-[12px] text-warning">
        ...
        <div className="px-3 pb-3">
          <div className="rounded-lg border border-border bg-muted/50 px-3 py-3">
            <div className="mb-2 flex flex-wrap items-center gap-2">
              <p className="min-w-0 flex-1 text-[12px] text-muted-foreground">{d.setup.pendingHelp}</p>
              <button
                type="button"
                onClick={() => void handleFinish()}
                disabled={verifying || checking || recordsLoading}
                className="inline-flex min-h-8 items-center gap-1.5 rounded-lg bg-foreground px-3 text-[12px] font-medium text-background hover:bg-foreground/90 disabled:opacity-50"
              >
                {verifying || checking ? <Loader2 className="size-3.5 animate-spin" /> : <RefreshCw className="size-3.5" />}
                {checking ? d.setup.checkingDns : verifying ? d.menu.verifying : d.menu.verify}
              </button>
            </div>
            {verifyHint || checkHint ? (
              <p className="mb-2 text-[12px] text-warning">{verifyHint || checkHint}</p>
            ) : null}
            {recordsLoading ? (
              ...
              <p className="py-1 text-[12px] text-muted-foreground">{d.records.none}</p>
            )}
          </div>
        </div>
      ) : null}
