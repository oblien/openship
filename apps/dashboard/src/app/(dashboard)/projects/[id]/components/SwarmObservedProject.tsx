...
function RoutingModePanel({ projectId, source, managementMode, onSourceChange }: {
  projectId: string;
  source: SwarmStackSource;
  ...
  const setMode = async (routingMode: SwarmStackSource["routingMode"]) => {
    ...
    if (routingMode === "openship-edge" && !window.confirm("Use the explicitly enabled OpenShip Swarm Edge for only services you mark exposed? This does not take over an existing router or bind ports automatically.")) return;
    setSaving(true);
    try {
      onSourceChange(await swarmApi.setRoutingMode(projectId, routingMode));
      showToast(routingMode === "external" ? "External routing restored. OpenShip will not modify router labels, domains, or TLS." : "OpenShip Edge routing selected. Enable the cluster Edge separately before deploying exposed services.", "success", "Docker Swarm routing");
    } catch (cause) {
      showToast(getApiErrorMessage(cause, "Unable to update the Swarm routing mode."), "error", "Docker Swarm routing");
    } finally {
      setSaving(false);
    }
  };
  return <section className="rounded-2xl border border-border/50 bg-card p-5"><div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="font-semibold text-foreground">Routing mode</h2><p className="mt-1 max-w-2xl text-sm leading-relaxed text-muted-foreground">{source.routingMode === "external" ? "External controller mode preserves router labels and leaves domains, TLS, and ports 80/443 outside OpenShip." : "OpenShip Edge attaches only services marked exposed to its overlay and routes them by stable Swarm service DNS. The Edge must be explicitly enabled on the selected manager."}</p></div><div className="flex gap-2"><button type="button" disabled={saving || source.routingMode === "external"} onClick={() => void setMode("external")} className="rounded-lg border border-border px-3 py-1.5 text-xs font-medium text-foreground hover:bg-muted disabled:opacity-60">External</button><button type="button" disabled={saving || managementMode !== "managed" || source.routingMode === "openship-edge"} onClick={() => void setMode("openship-edge")} className="rounded-lg border border-primary/30 bg-primary/5 px-3 py-1.5 text-xs font-medium text-primary hover:bg-primary/10 disabled:opacity-60">Use OpenShip Edge</button></div></div>{managementMode !== "managed" && <p className="mt-3 text-xs text-muted-foreground">Claim management before enabling the Edge option.</p>}</section>;
}
...
function SourceManagementPanel({
  ...
  const hasHighStorageRisk = preview?.compatibility.warnings.some((issue) => [
    "SWARM_STORAGE_BIND_UNVERIFIED",
    "SWARM_STORAGE_LOCAL_VOLUME_UNPINNED",
  ].includes(issue.code)) ?? false;
  const hasRisk = hasHighStorageRisk || (preview?.changes.some((change) => ["service-remove", "network-port-change", "config-secret-reference-change"].includes(change.kind)) ?? false);
  return <section className="rounded-2xl border border-border/50 bg-card p-5">
    <div className="flex flex-wrap items-start justify-between gap-4">
