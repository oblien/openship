  ...
  return (
    <div className="rounded-2xl border border-border/50 bg-card">
        ...
        {repository && (
          <div className="flex items-center justify-between gap-3 border-b border-border/40 py-3 last:border-0">
            ...
            >
              ...
              <UiIcon name="external-link" className="size-3.5 text-muted-foreground" />
            </a>
          </div>
        )}
      </div>
      ...
      <div className="flex items-center justify-between gap-3 border-t border-border/50 px-5 py-4">
        ...
        >
          {checking ? <UiIcon name="spinner" className="size-4 animate-spin" /> : <UiIcon name="refresh" className="size-4" />}
          {s.checkForUpdates}
        </button>
        ...
        {inProgress ? (
          ...
          !loading && (
            <span className="inline-flex items-center gap-1.5 text-xs font-medium text-success">
              <UiIcon name="check" className="size-3.5" /> {s.upToDate}
            </span>
          )
        )}
      </div>
