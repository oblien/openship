          ...
          <div className="flex items-center justify-between mb-2">
            ...
            <button
              ...
              className={`
                ...
                ${projectData?.auto_deploy ? 'bg-primary' : 'bg-muted'}
          ...
          {isEditingBranch ? (
            <div className="space-y-3">
              <select
                ...
                className="w-full px-5 py-3 border border-border rounded-[15px] outline-none text-foreground bg-card focus:ring-2 focus:ring-black focus:border-transparent cursor-text"
              >
                {branches.map((branch) => (
            ...
            <div className="relative p-4 bg-card rounded-xl group hover:bg-muted/60 transition-all border border-border">
              <div className="flex items-center gap-3">
                <GitBranch className="w-5 h-5 text-muted-foreground" />
                <p className="text-sm font-medium text-foreground flex-1 font-mono">{projectData?.branch || 'main'}</p>
              </div>
            </div>
          )}
        </div>
          ...
          {projectData?.recentCommits && projectData.recentCommits.length > 0 ? (
            <div className="space-y-3">
              {projectData.recentCommits.slice(0, 3).map((commit: any, i: number) => (
                <div key={i} className="p-3 bg-muted/60 rounded-lg border border-border">
