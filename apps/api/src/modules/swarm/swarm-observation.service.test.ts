...
function snapshot(clusterId = "cluster-a"): SwarmDiscoverySnapshot {
  return {
    manager: { engineVersion: null, apiVersion: null, localNodeState: "active", controlAvailable: true, clusterId, nodeId: "node", nodeAddress: null, managerAddress: null },
    nodes: [], stacks: [{ name: "blog", serviceIds: ["svc"], serviceNames: ["web"] }],
    services: [{
      id: "svc", name: "blog_web", sourceServiceName: "web", stackName: "blog", specVersion: 2,
      mode: "replicated", desiredReplicas: 2, image: "nginx@sha256:new", labels: {}, endpointMode: null,
...
function fixture(discovery = snapshot()) {
  const stack = {
    id: "swarm-blog", projectId: "proj-blog", organizationId: "org-a", managerServerId: "server-a", clusterId: "cluster-a",
    stackName: "blog", managementMode: "observe", sourceKind: "adopted", sourceStatus: "missing",
    driftStatus: "unknown", driftDetails: {}, lastObservedDigest: "sha256:old", lastObservedAt: null,
...
describe("Swarm observation refresh", () => {
  it("records redacted external drift and refreshes service projections without a stack operation", async () => {
    ...
    expect(test.syncProjections).toHaveBeenCalledWith("proj-blog", [expect.objectContaining({ sourceServiceName: "web", observedServiceId: "svc" })]);
  });
  ...
  it("surfaces a manager cluster identity change instead of treating it as ordinary drift", async () => {
    ...
    await expect(test.service.refresh("proj-blog", "org-a")).rejects.toMatchObject({ code: "SWARM_CLUSTER_MISMATCH", statusCode: 409 });
