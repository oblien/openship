...
import { extractChangedFiles, routeServicesByChanges } from "../github/webhook-changed-files";
...
import type { VcsPushPayload } from "./vcs.types";
...
export async function handlePush(
  ...
  if (!ref?.startsWith("refs/heads/")) {
    return {
      success: true,
      event: "push",
      message: `Ignoring non-branch ref: ${ref ?? "unknown"}`,
    };
  }
...
async function deployProjectFromPush(p: Project, input: BranchDeploymentTrigger) {
  ...
  if (input.payload) {
    const extracted = await extractChangedFiles(input.payload, {
      isMonorepo: p.framework === "monorepo" || routableServices.length > 0,
      ...
      compareCommits: async (owner, repo, base, head) =>
        VcsStrategyFactory.getStrategy(input.provider).compareCommits(
          webhookActorCtx(
            actorUserId,
            p.organizationId ?? "",
            `webhook:${input.provider}-compare-commits`,
          ),
          owner,
          repo,
          base,
    ...
    // double-fire force.
    const consumed = await repos.project.consumeForceDeployNext(p.id).catch(() => false);
    if (consumed) {
      forceAll = true;
  ...
  if (!triggered?.skipped && triggered?.deployment?.id && (changedPaths || changedPathsTruncated)) {
    ...
    await repos.deployment
      ...
      .catch((err: unknown) => {
        console.warn(`[GitHub Webhook] failed to persist changedPaths for ${deploymentId}:`, err);
      });
  }
...
async function resolveDefaultBranch(
  ...
  const unbranchedProject = projects.find((p) => !p.gitBranch?.trim() && p.gitOwner && p.gitRepo);
  ...
  try {
    ...
    const repository = await VcsStrategyFactory.getStrategy(input.provider).getRepository(
      webhookActorCtx(
        owner.userId,
        unbranchedProject.organizationId,
        `webhook:${input.provider}-resolve-default-branch`,
      ),
      input.owner,
      input.repo,
    );
