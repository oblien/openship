...
import type { WebhookVerifyResult } from "@repo/platform/engine/modules/webhooks/webhook.types";
...
export function verifyAzureBasicAuth(
  ...
  const password = parseBasicPassword(headers.authorization ?? headers.Authorization);
  if (!password) {
