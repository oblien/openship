...
/** Code pointer is the mounted-release row; absent when that lane is off. */
export function activeCodeReleaseDeploymentId(
  project: Pick<Project, "activeReleaseDeploymentId" | "mountedRelease">,
): string | null {
  return mountedReleaseConfig(project) ? (project.activeReleaseDeploymentId ?? null) : null;
}

/** Thin live-state: active runtime deployment + active code release + bound server. */
...
export async function readProjectLiveState(project: Project): Promise<ProjectLiveState> {
  ...
  const metaServerId =
    runtimeDep && typeof runtimeDep.meta === "object" && runtimeDep.meta
      ? ((runtimeDep.meta as { serverId?: string }).serverId ?? null)
      : null;
  const serverId = metaServerId ?? project.serverId ?? null;
  const server = serverId
    ? ((await repos.server.getInOrganization(serverId, project.organizationId).catch(() => null)) ?? null)
    : null;
