...
function discovery(services = true): SwarmDiscoverySnapshot {
  ...
  return {
    ...
    stacks: services ? [{ name: "blog", serviceIds: ["svc-web", "svc-worker"], serviceNames: ["web", "worker"] }] : [],
    services: services ? [
      { id: "svc-web", name: "blog_web", sourceServiceName: "web", stackName: "blog", specVersion: 4, mode: "replicated", desiredReplicas: 1, image: "nginx:1.27-alpine", labels, endpointMode: null, placement: null, resources: null, updateConfig: null, rollbackConfig: null, restartPolicy: null, networks: [], configs: [], secrets: [], publishedPorts: [], updateState: null, updateMessage: null },
      { id: "svc-worker", name: "blog_worker", sourceServiceName: "worker", stackName: "blog", specVersion: 3, mode: "replicated", desiredReplicas: 1, image: "busybox:1.36", labels, endpointMode: null, placement: null, resources: null, updateConfig: null, rollbackConfig: null, restartPolicy: null, networks: [], configs: [], secrets: [], publishedPorts: [], updateState: null, updateMessage: null },
    ] : [],
    tasks: services ? [
      { id: "task-web", serviceId: "svc-web", serviceName: "blog_web", slot: 1, nodeId: "node-a", nodeName: "manager", desiredState: "Running", currentState: "Running 1 second ago", error: null, image: "nginx:1.27-alpine", updatedAt: null, observedAt: "2026-07-30T00:00:00.000Z" },
      { id: "task-worker", serviceId: "svc-worker", serviceName: "blog_worker", slot: 1, nodeId: "node-a", nodeName: "manager", desiredState: "Running", currentState: "Running 1 second ago", error: null, image: "busybox:1.36", updatedAt: null, observedAt: "2026-07-30T00:00:00.000Z" },
    ] : [],
...
function fixture(options: {
  ...
  return { service, events, createRevision, updateRevision, updateStack, syncProjections, createServiceDeployments, deployStack, log, logger };
}
...
describe("managed Swarm deploy", () => {
  it("records a revision before applying and persists stack/service runtime references after manager convergence", async () => {
    ...
    await expect(test.service.deploy({ project, deployment, environment: { API_TOKEN: "top-secret" }, logger: test.logger }))
      .resolves.toMatchObject({ state: "ready", revisionId: "revision-1", runtimeRef: { kind: "swarm-stack", stackName: "blog" } });
    ...
    expect(test.deployStack).toHaveBeenCalledWith(expect.objectContaining({
      stackName: "blog", resolveImage: "always", prune: false,
    }));
    expect(test.updateRevision).toHaveBeenCalledWith("revision-1", "org-a", expect.objectContaining({ applyStatus: "ready" }));
    expect(test.updateStack).toHaveBeenCalledWith("swarm-blog", "org-a", expect.objectContaining({
      lastAppliedRevisionId: "revision-1", driftStatus: "clean",
    }));
    expect(test.createServiceDeployments).toHaveBeenCalledWith(expect.arrayContaining([
      expect.objectContaining({ serviceName: "web", status: "success", runtimeRef: expect.objectContaining({ kind: "swarm-service", serviceId: "svc-web" }) }),
      expect.objectContaining({ serviceName: "worker", status: "success", runtimeRef: expect.objectContaining({ kind: "swarm-service", serviceId: "svc-worker" }) }),
    ]));
  ...
  it("keeps a possibly accepted apply reconciling when manager access drops after docker stack deploy", async () => {
    ...
    await expect(test.service.deploy({ project, deployment, environment: {}, logger: test.logger }))
      .resolves.toMatchObject({ state: "reconciling", warningMessage: expect.stringContaining("connection dropped") });
