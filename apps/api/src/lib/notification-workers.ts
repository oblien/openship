...
async function sendWebhook(
  ...
  // SSRF guard: the webhook URL is user-supplied but the request is made by
  // the server, so an unchecked URL could reach cloud metadata
  // (169.254.169.254) or internal-only services. assertPublicHttpsUrl resolves
  // the host and refuses if the literal host OR any resolved address is
  // private / loopback / link-local / metadata — resolving (not just matching
  // the hostname text) is what stops DNS-rebinding and numeric-IP encodings.
  // See lib/ssrf-guard.
