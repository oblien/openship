...
const h = vi.hoisted(() => ({
  ...
  backupPolicyCreate: vi.fn(),
  backupPolicyUpdate: vi.fn(),
  backupPolicyDelete: vi.fn(),
  backupRetentionLock: vi.fn(),
  backupEnqueue: vi.fn(),
  backupPrepare: vi.fn(),
  backupApply: vi.fn(),
  backupCancel: vi.fn(),
  backupSyncSchedule: vi.fn(),
  backupRemoveSchedule: vi.fn(),
  destinationCreate: vi.fn(),
  destinationUpdate: vi.fn(),
  destinationDelete: vi.fn(),
  destinationVerify: vi.fn(),
  destinationProbe: vi.fn(),
  deployments: new Map<string, ReturnType<typeof storedDeployment>>(),
  deploymentContainerInfo: vi.fn(),
  deploymentContainerUsage: vi.fn(),
  deploymentPendingActions: vi.fn(),
  ...
  serverUpdate: vi.fn(),
  serverRemove: vi.fn(),
  serverWorkloads: vi.fn(),
  serverExec: vi.fn(),
  serverWithExecutor: vi.fn(),
  serverModuleScan: vi.fn(),
  serverModuleApply: vi.fn(),
  serverRateRead: vi.fn(),
  serverRateApply: vi.fn(),
  componentCheck: vi.fn(),
  installDocker: vi.fn(),
  installEdge: vi.fn(),
  removeEdge: vi.fn(),
  deliverEdge: vi.fn(),
  refreshServerContainer: vi.fn(),
  monitorExec: vi.fn(),
  serverRetain: vi.fn(),
  serverRelease: vi.fn(),
  containerRows: new Map<string, ReturnType<typeof serverContainerFixture>>(),
  containerUpsert: vi.fn(),
  containerProgress: vi.fn(),
  reconcileEdge: vi.fn(),
  mailRecord: vi.fn(),
  domainCreate: vi.fn(),
  domainList: vi.fn(),
  ...
  env: {
    CLOUD_MODE: false,
    DEPLOY_MODE: "docker",
    BETTER_AUTH_SECRET: "server-parity-test-encryption-secret-32-bytes",
    OPENSHIP_PRODUCT: "platform",
    OPENSHIP_AUTH_MODE: undefined as string | undefined,
...
vi.mock("@repo/db", () => ({
  ...
  repos: {
    ...
    instanceSettings: {
      ...
      upsert: h.settingsUpsert,
      delete: h.settingsDelete,
    },
    serverTunnel: {
      get: async (id: string) => h.tunnels.get(id),
      getByTarget: async (serverId: string, remotePort: number, remoteHost: string) =>
        [...h.tunnels.values()].find(
          (row) =>
            row.serverId === serverId &&
            row.remotePort === remotePort &&
            row.remoteHost === remoteHost,
        ),
      listByServer: async (id: string) =>
        [...h.tunnels.values()].filter((row) => row.serverId === id),
      upsert: h.tunnelUpsert,
      remove: h.tunnelRemove,
    },
    member: { find: async (org: string, user: string) => h.members.get(`${org}:${user}`) },
    project: {
      findById: async (id: string) => h.projects.get(id),
      findDraftByAppTemplate: h.appDraft,
      listEnvVars: async (id: string, environment: string, service?: string) =>
        service
          ? (h.appEnv.get(`${id}:${environment}:${service}`) ?? [])
          : [...h.appEnv.entries()]
              .filter(([key]) => key.startsWith(`${id}:${environment}:`))
              .flatMap(([key, rows]) =>
                rows.map((row) => ({
                  ...row,
                  serviceId: key.slice(`${id}:${environment}:`.length),
                })),
              ),
      mergeEnvVars: h.appEnvMerge,
      countActiveByServer: async () => ({ "server-a": 2 }),
      listActiveByServer: h.serverWorkloads,
    },
    customAppTemplate: {
      listByOrg: async (org: string) =>
        [...h.customApps.values()].filter((row) => row.organizationId === org),
      ...
      upsert: async (row: {
        ...
        h.customApps.set(`${row.organizationId}:${row.appId}`, {
          ...row,
          updatedAt: new Date("2026-09-12T00:00:00Z"),
        });
      },
      deleteByAppId: async (org: string, id: string) => {
        h.customApps.delete(`${org}:${id}`);
      },
    },
    backupDestination: {
      findById: async (id: string) => h.destinations.get(id),
      listByOrganization: async (org: string) =>
        [...h.destinations.values()].filter((row) => row.organizationId === org),
      findByNameInOrganization: async (org: string, name: string) =>
        [...h.destinations.values()].find((row) => row.organizationId === org && row.name === name),
      create: h.destinationCreate,
      update: h.destinationUpdate,
      softDelete: h.destinationDelete,
      setLastVerified: h.destinationVerify,
    },
    backupRun: {
      statsByDestination: async () => [],
      findById: async (id: string) => h.backupRuns.get(id),
      listByOrganization: async (
        org: string,
        opts: { projectId?: string; serviceId?: string; limit?: number },
      ) =>
        [...h.backupRuns.values()]
          .filter(
            (row) =>
              ...
              (!opts.serviceId || row.serviceId === opts.serviceId),
          )
          .slice(0, opts.limit),
      setRetentionLock: h.backupRetentionLock,
    },
    backupPolicy: {
      listByDestination: async () => [],
      findById: async (id: string) => h.backupPolicies.get(id),
      listByProject: async (id: string) =>
        [...h.backupPolicies.values()].filter((row) => row.projectId === id),
      create: h.backupPolicyCreate,
      update: h.backupPolicyUpdate,
      softDelete: h.backupPolicyDelete,
    },
    backupRestore: { findById: async (id: string) => h.backupRestores.get(id) },
    deployment: { findById: async (id: string) => h.deployments.get(id) },
    service: {
      findById: async (id: string) => h.services.get(id),
      listByProject: async (id: string) =>
        [...h.services.values()].filter((row) => row.projectId === id),
    },
    domain: {
      findById: async (id: string) => h.domains.get(id),
      findByHostname: async (hostname: string) =>
        [...h.domains.values()].find((row) => row.hostname === hostname),
      listByProject: async (id: string) =>
        [...h.domains.values()].filter((row) => row.projectId === id),
    },
    server: {
      list: async () => [...h.servers.values()],
      get: async (id: string) => h.servers.get(id),
      getInOrganization: async (id: string, org: string) =>
        h.servers.get(id)?.organizationId === org ? h.servers.get(id) : undefined,
      listByOrganization: async (org: string) =>
        [...h.servers.values()].filter((row) => row.organizationId === org),
      create: h.serverCreate,
      update: h.serverUpdate,
      delete: h.serverRemove,
    },
    mailServer: { get: h.mailRecord },
    serverContainerStatus: {
      listByServer: async (id: string) =>
        [...h.containerRows.values()].filter((row) => row.serverId === id),
      listByOrg: async (id: string) =>
        [...h.containerRows.values()].filter((row) => row.organizationId === id),
      listBehindByOrg: async (id: string) =>
        [...h.containerRows.values()].filter((row) => row.organizationId === id && row.behind),
      setInProgress: h.containerProgress,
      upsert: h.containerUpsert,
      remove: vi.fn(async () => {}),
    },
    routeRule: {
      get: h.ruleGet,
...
vi.mock("@repo/platform/engine/lib/instance-authorization", async () => {
  ...
  return {
    instanceAuthorization: createInstanceAuthorization({
      findUserRole: async (userId) => h.instanceRoles.get(userId),
    }),
  };
});
vi.mock("@repo/platform/engine/lib/mail", async (importOriginal) => ({
  ...
  sendInstanceTestEmail: h.mailTest,
  canSendMail: h.mailAvailable,
  invalidateInstanceTransportCache: h.invalidateMailCache,
}));
...
vi.mock("@repo/platform/engine/lib/edge-orphans.service", () => ({
  scanEdgeOrphans: h.orphanScan,
  removeEdgeOrphan: h.orphanRemove,
}));
...
vi.mock("@repo/platform/engine/lib/ssh-tunnel", async (importOriginal) => ({
  ...(await importOriginal<typeof import("@repo/platform/engine/lib/ssh-tunnel")>()),
  tunnelForward: h.tunnelForward,
}));
...
vi.mock("@repo/platform/engine/modules/deployments/deployment.service", async (importOriginal) => ({
  ...(await importOriginal<
    typeof import("@repo/platform/engine/modules/deployments/deployment.service")
  >()),
  getContainerInfo: h.deploymentContainerInfo,
  getContainerUsage: h.deploymentContainerUsage,
}));
vi.mock(
  "@repo/platform/engine/modules/projects/pending-actions.service",
  async (importOriginal) => ({
    ...(await importOriginal<
      typeof import("@repo/platform/engine/modules/projects/pending-actions.service")
    >()),
...
vi.mock("@repo/platform/engine/modules/services/service.service", () => ({
  createService: h.serviceCreate,
  getService: h.serviceGet,
  updateService: h.serviceUpdate,
  revealServiceEnv: h.serviceReveal,
  restartServiceContainer: h.serviceRestart,
  execInServiceContainer: h.serviceExec,
}));
vi.mock("@repo/platform/engine/modules/domains/domain.service", () => ({
  addDomain: h.domainCreate,
  listDomains: h.domainList,
  getDomain: h.domainGet,
  verifyDomain: h.domainVerify,
  getDomainRecords: h.domainRecords,
  uploadDomainCert: h.domainUpload,
  verifyPendingDomains: h.domainPending,
}));
vi.mock("@repo/platform/engine/modules/dns/dns-credential.service", () => ({
  getCredential: h.dnsGet,
  listCredentials: h.dnsList,
  addCredential: h.dnsAdd,
  removeCredential: h.dnsRemove,
  resolveDnsManager: h.dnsResolve,
}));
vi.mock("@repo/platform/engine/modules/credentials/credential.service", () => ({
  getCredential: h.credentialGet,
  listCredentials: h.credentialList,
  createCredential: h.credentialCreate,
  updateCredential: h.credentialUpdate,
  deleteCredential: h.credentialRemove,
  verifyCredential: h.credentialVerify,
}));
...
vi.mock("@repo/platform/engine/modules/route-rules/route-rule.service", () => ({
  pushProjectRulesResolved: h.rulePush,
}));
...
vi.mock("@repo/platform/engine/lib/geo-ip", () => ({
  primeGeo: vi.fn(async () => {}),
  countryForIp: () => null,
}));
vi.mock("@repo/platform/engine/lib/host-capacity", () => ({
  invalidateHostCapacity: vi.fn(async () => {}),
  getTrustedHostCapacity: h.hostCapacity,
}));
vi.mock("@repo/platform/engine/lib/openresty-paths", () => ({
  invalidateOpenRestyPaths: vi.fn(async () => {}),
  withOpenRestyRouting: async (_id: string, work: (routing: unknown) => unknown) =>
    work({ getRateLimitConfig: h.serverRateRead, applyRateLimit: h.serverRateApply }),
}));
vi.mock("@repo/platform/engine/lib/ssh-manager", () => ({
  sshManager: {
    withExecutor: h.serverWithExecutor,
    invalidate: vi.fn(),
    acquire: async () => ({ exec: h.monitorExec }),
    retain: h.serverRetain,
    release: h.serverRelease,
  },
}));
vi.mock("@repo/platform/engine/lib/agent-exec", () => ({ execOnHost: h.serverExec }));
vi.mock("@repo/platform/engine/modules/system/server-modules.service", () => ({
  scanServer: h.serverModuleScan,
  applyServerModule: h.serverModuleApply,
}));
vi.mock("@repo/adapters", async (importOriginal) => {
  ...
  return {
    ...
    COMPONENT_INSTALLERS: {
      ...actual.COMPONENT_INSTALLERS,
      docker: h.installDocker,
      edge: h.installEdge,
    },
    COMPONENT_UNINSTALLERS: { ...actual.COMPONENT_UNINSTALLERS, edge: h.removeEdge },
    ensureEdge: async (
      _executor: unknown,
      work: (prompt: unknown) => unknown,
      options: { promptUser: unknown },
    ) => ({ migrated: false, value: await work(options.promptUser) }),
    recoverInterruptedTakeover: async () => {},
  };
});
vi.mock("@repo/platform/engine/lib/deliver-managed-image", () => ({
  deliverManagedImage: h.deliverEdge,
}));
vi.mock(
  "@repo/platform/engine/modules/system/server-containers.service",
  async (importOriginal) => ({
    ...(await importOriginal<
      typeof import("@repo/platform/engine/modules/system/server-containers.service")
    >()),
    refreshServerContainer: h.refreshServerContainer,
  }),
);
vi.mock("@repo/platform/engine/lib/edge-reconcile", async (importOriginal) => ({
  ...(await importOriginal<typeof import("@repo/platform/engine/lib/edge-reconcile")>()),
  reconcileServerEdge: h.reconcileEdge,
}));
...
describe("deployment preparation HTTP/native parity", () => {
  ...
  it.each(["owner", "admin", "member"])(
    "retains scanner inputs and public output for a %s",
    async (role) => {
      ...
      expect(result).toMatchObject({
        ...
        composePath: "deploy/compose.yaml",
        productionPaths: ["dist"],
        rootEnv: { ROOT_TOKEN: ENV_MASK, EMPTY: "" },
        openshipEnvKeys: ["DECLARED_TOKEN"],
        services: [{ name: "web", environment: { API_TOKEN: ENV_MASK, EMPTY: "" } }],
        missingRequiredEnv: info.missingRequiredEnv,
        configDiagnostics: info.configDiagnostics,
      });
      ...
      for (const [source] of h.localInfo.mock.calls) {
        ...
        expect(source.ctx).toMatchObject({
          userId: "alice",
          organizationId: "org-a",
  ...
  it.each(["restricted member", "scoped token"])(
    "preserves the API's collection rule for a %s with a stored project-wide write grant",
    async (principal) => {
      if (principal === "restricted member") {
        ...
        h.identity = {
          ...alice,
          tokenScope: { tokenId: "prepare-token" },
      ...
      for (const deployments of [remote().deployments, local.deployments]) {
        await expect(deployments.prepare({ owner: "acme", repo: "app" })).rejects.toMatchObject({
          code: "NOT_FOUND",
          statusCode: 404,
        });
      }
  ...
  it("retains the cloud refusal for local filesystem preparation", async () => {
    ...
    const local = await native();
    for (const deployments of [remote().deployments, local.deployments]) {
      await expect(deployments.prepare({ source: "local", path: "/unread" })).rejects.toMatchObject(
        { statusCode: 403 },
      );
    }
    expect(h.localInfo).not.toHaveBeenCalled();
  ...
  it("returns editable source values in one authorized HTTP or native preparation", async () => {
    ...
    const input = {
      ...
      branch: "preview",
      composePath: " deploy/compose.yaml ",
      env: { OVERRIDE: "typed-value" },
      includeEnv: true,
    };
    for (const deployments of [remote().deployments, local.deployments]) {
      expect(await deployments.prepare(input)).toMatchObject({
        services: [
    ...
    expect(h.sourceContent).toHaveBeenCalledWith(
      expect.anything(),
    ...
    for (const deployments of [remote().deployments, local.deployments]) {
      expect(await deployments.prepare({ ...input, includeEnv: false })).toMatchObject({
        services: [
          { name: "web", environment: { API_TOKEN: ENV_MASK } },
          {
            name: "db",
            environment: { DB_PASSWORD: ENV_MASK },
            buildArgs: { TOKEN: ENV_MASK, INHERITED: null, EMPTY: "" },
          },
        ],
      });
    }
  ...
  it("does not turn deploy-only source access into permission to reveal file values", async () => {
    ...
    for (const deployments of [remote().deployments, local.deployments]) {
      await expect(
        deployments.prepare({ owner: "acme", repo: "app", includeEnv: true }),
    ...
    for (const deployments of [remote().deployments, local.deployments]) {
      expect(
        JSON.stringify(await deployments.prepare({ owner: "acme", repo: "app" })),
  ...
  it("rejects invalid disclosure options before scanning", async () => {
    ...
    for (const deployments of [remote().deployments, local.deployments]) {
      for (const includeEnv of [null, "true", 1, {}, []]) {
        await expect(
          deployments.prepare({ owner: "acme", repo: "app", includeEnv: includeEnv as boolean }),
  ...
  it("rejects editable local scans in cloud mode", async () => {
    ...
    for (const deployments of [remote().deployments, local.deployments]) {
      await expect(
        deployments.prepare({ source: "local", path: "/unread", includeEnv: true }),
...
describe("HTTP/native service parity", () => {
  ...
  it("uses the same strict write, audit, and output for a parent-scoped creator", async () => {
    ...
    const input = {
      name: "web",
      environment: { TOKEN: "never-audit-this-value" },
      commandArgv: ["sh", "-c", "echo ready"],
    };
    expect(await remote().services.create("project-a", input)).toEqual(
      await local.services.create("project-a", input),
    );
    ...
    await expect(
      remote().services.update("project-a", "service-a", { kind: "monorepo" } as never),
    ).rejects.toMatchObject({ code: "VALIDATION_ERROR" });
    await expect(
      local.services.update("project-a", "service-a", { kind: "monorepo" } as never),
    ).rejects.toMatchObject({ code: "VALIDATION_ERROR" });
  ...
  it("rejects a forged parent and cross-tenant service in both facades", async () => {
    ...
    for (const services of [remote().services, local.services]) {
      await expect(services.get("project-sibling", "service-a")).rejects.toMatchObject({
        code: "NOT_FOUND",
      });
      await expect(services.get("project-b", "service-b")).rejects.toMatchObject({
        code: "NOT_FOUND",
      });
    }
  ...
  it("reveals only requested keys and records only their names", async () => {
    ...
    for (const services of [remote().services, local.services]) {
      expect(
        await services.revealEnv("project-a", "service-a", { keys: ["TOKEN", "toString"] }),
      ).toEqual({ TOKEN: "one-secret" });
    }
    expect(h.audit).toHaveBeenCalledTimes(2);
  ...
  it("preserves stale-env refusal fields and accepts a bodyless force query", async () => {
    h.serviceRestart.mockRejectedValue(
      new ServiceConfigStaleError("Pending env changes", ["TOKEN"], "web"),
    );
    const local = await native();
    for (const services of [remote().services, local.services]) {
      await expect(services.restart("project-a", "service-a")).rejects.toMatchObject({
        statusCode: 409,
        code: "SERVICE_CONFIG_STALE",
        details: { staleEnvKeys: ["TOKEN"], serviceName: "web" },
      });
    }
    h.serviceRestart.mockResolvedValue({ containerId: "container" });
    const response = await app.request(
      "/api/projects/project-a/services/service-a/restart?force=1",
  ...
  it("audits bounded exec metadata without disclosing command output", async () => {
    h.serviceExec.mockResolvedValue({
      exitCode: 0,
    ...
    const local = await native();
    expect(await local.services.exec("project-a", "service-a", { command: "  pwd  " })).toEqual(
      await remote().services.exec("project-a", "service-a", { command: "  pwd  " }),
    );
    expect(h.serviceExec.mock.calls[0]![3]).toEqual({ command: "pwd" });
    expect(h.audit).toHaveBeenCalledTimes(2);
    expect(h.audit.mock.calls[0]![0]).toMatchObject({
      eventType: "service.exec",
      after: { command: "pwd", exitCode: 0, outputBytes: 21 },
    });
...
describe("app HTTP/native parity through retained catalog, installer, and settings services", () => {
  const remote = (organizationId = "org-a") =>
    new OpenshipClient({
      baseUrl: "http://openship.test",
      organizationId,
      fetch: ((url, init) => app.request(url as string, init)) as typeof fetch,
  ...
  const definition: AppTemplate = {
    id: "parity-app",
    name: "Parity App",
    description: "Shared installer fixture",
    kind: "template",
    logo: "box",
    category: "other",
    verified: true,
    ...
    connection: {
      outputs: [{ id: "token", label: "Token", source: "env:app:TOKEN", secret: true }],
    },
  };
  beforeEach(() => {
    vi.stubGlobal("fetch", async () => Response.json({ apps: [] }));
    h.customApps.set(`org-a:${definition.id}`, {
      organizationId: "org-a",
      appId: definition.id,
      template: definition,
  ...
  afterEach(() => {
    vi.unstubAllGlobals();
    h.projectCreate.mockReset();
    h.serviceCreate.mockReset();
  });
  ...
  it("uses authoritative upload validation, unverified provenance, and organization-scoped storage", async () => {
    ...
    for (const apps of [local.apps, remote().apps]) {
      ...
      expect((await apps.getCatalogEntry(definition.id)).template).toMatchObject({
        verified: false,
        custom: true,
      });
      expect(await apps.listCustom()).toEqual([
        { appId: definition.id, name: definition.name, updatedAt: "2026-09-12T00:00:00.000Z" },
      ]);
      await expect(
        apps.saveCustom({
          ...definition,
          connection: { outputs: [{ id: "bad", label: "Bad", source: "env:missing:TOKEN" }] },
        }),
      ).rejects.toMatchObject({ code: "VALIDATION_ERROR" });
    }
    ...
    for (const apps of [(await native("org-b")).apps, remote("org-b").apps]) {
      expect(await apps.listCustom()).toEqual([]);
      await expect(apps.getCatalogEntry(definition.id)).rejects.toMatchObject({
        code: "NOT_FOUND",
      });
    }
  ...
  it("passes create-only token authority into the existing project-creation transaction", async () => {
    ...
    for (const args of h.projectCreate.mock.calls) {
      expect(args).toEqual([
        expect.objectContaining({ appTemplateId: definition.id, projectType: "services" }),
        "org-a",
        { tokenId: "app-token" },
      ]);
    }
    expect(
      h.serviceCreate.mock.calls.every(([ctx]) => ctx.role === "restricted" && !ctx.hono),
    ).toBe(true);
    expect(h.audit).toHaveBeenCalledTimes(2);
    for (const apps of [local.apps, remote().apps])
      await expect(apps.saveCustom(definition)).rejects.toMatchObject({ code: "NOT_FOUND" });
  });
  ...
  it("refuses draft adoption before the retained installer mutates any project or service", async () => {
    ...
    for (const apps of [(await native()).apps, remote().apps]) {
      await expect(apps.install({ templateId: definition.id })).rejects.toMatchObject({
        code: "NOT_FOUND",
      });
    }
  ...
  it("retains settings masking, blank-secret preservation, and one audit per write or credential reveal", async () => {
    ...
    for (const projects of [local.projects, remote().projects]) {
      expect(
        (await projects.getAppSettings("project-a")).values.find((row) => row.key === "TOKEN"),
      ).toMatchObject({ secret: true, set: true, value: "" });
      expect(
        await projects.updateAppSettings("project-a", {
          changes: [{ service: "app", key: "TOKEN", value: "replacement-app-secret" }],
        }),
      ).toEqual({ count: 1, requiresRedeploy: false });
      expect(
        await projects.updateAppSettings("project-a", {
          changes: [{ service: "app", key: "TOKEN", value: "" }],
        }),
      ).toEqual({ count: 0, requiresRedeploy: false });
      expect((await projects.getAppConnection("project-a")).outputs[0]?.value).toBe(
        "replacement-app-secret",
      );
      expect(JSON.stringify(await projects.getAppSettings("project-a"))).not.toContain(
        "replacement-app-secret",
      );
    }
    ...
    expect(h.audit.mock.calls[0]![0].after).toMatchObject({
      fields: [{ service: "app", key: "TOKEN" }],
    });
    expect(
      h.audit.mock.calls
        .filter(([row]) => row.after?.operation === "getAppConnection")
        .map(([row]) => row.after.revealedOutputs),
    ).toEqual([["token"], ["token"]]);
    h.identity = { ...alice, credential: { organizationId: "org-a", readOnly: true } };
    for (const projects of [local.projects, remote().projects]) {
      await expect(projects.getAppConnection("project-a")).rejects.toMatchObject({
        code: "TOKEN_READ_ONLY",
      });
  ...
  it("checks server access before capacity probes and respects native host policy", async () => {
    ...
    const local = await native();
    for (const apps of [local.apps, remote().apps]) {
      await expect(apps.hostFit(definition.id, { serverId: "server-b" })).rejects.toMatchObject({
        code: "NOT_FOUND",
      });
      await expect(apps.hostFit(definition.id, { serverId: "server-a" })).rejects.toMatchObject({
        code: "NOT_FOUND",
      });
    }
    expect(h.hostCapacity).not.toHaveBeenCalled();
    h.grants.set("org-a:alice:server:server-a", { permissions: ["read"] });
    expect(await local.apps.hostFit(definition.id, { serverId: "server-a" })).toEqual(
      await remote().apps.hostFit(definition.id, { serverId: "server-a" }),
    );
    ...
    expect(await local.apps.hostFit(definition.id)).toMatchObject({
  ...
  it("preserves installer refusals as structured 400 responses in both interfaces", async () => {
    for (const apps of [(await native()).apps, remote().apps]) {
      await expect(apps.install({ templateId: "unknown-template" })).rejects.toMatchObject({
        statusCode: 400,
...
describe("backup destination HTTP/native parity through retained storage services", () => {
  ...
  function destination(id: string, organizationId: string): StoredBackupDestination {
    return {
      ...backupDestinationFixture(id),
      organizationId,
      accessKeyIdEnc: encryptSecretField("storage-access-key"),
      secretAccessKeyEnc: encryptSecretField("storage-private-secret"),
  ...
  beforeEach(() => {
    ...
    h.destinationDelete.mockImplementation(async (id) => {
  ...
  it("shares secret-free presentation and fixed tenant authority, including direct-ID grants", async () => {
    ...
    for (const destinations of [local.backupDestinations, remote().backupDestinations]) {
      expect(await destinations.usage("destination-a")).toEqual({
  ...
  it("reuses credential encryption and records only create metadata", async () => {
    ...
    const input = {
      ...
      secretAccessKey: "fresh-storage-secret",
    };
    ...
    expect(decryptSecretField(h.destinations.get(created.id)!.secretAccessKeyEnc)).toBe(
      "fresh-storage-secret",
    );
    ...
    expect(h.audit.mock.calls[0]![0]).toMatchObject({
      eventType: "backup_destination:write",
      resourceId: created.id,
      after: { operation: "create", name: input.name, kind: input.kind },
    });
  ...
  it("preserves omitted credentials, clears explicit nulls, and audits field names once", async () => {
    ...
    expect(
      await local.backupDestinations.update("destination-a", { name: "Renamed" }),
    ).toMatchObject({ hasSecretAccessKey: true });
    expect(h.destinations.get("destination-a")!.secretAccessKeyEnc).toBe(original);
    expect(
      await remote().backupDestinations.update("destination-a", {
        secretAccessKey: "replacement-storage-secret",
      }),
    ).toMatchObject({ hasAccessKeyId: true, hasSecretAccessKey: true });
    expect(decryptSecretField(h.destinations.get("destination-a")!.secretAccessKeyEnc)).toBe(
      "replacement-storage-secret",
    );
    expect(
      await local.backupDestinations.update("destination-a", { secretAccessKey: null }),
    ).toMatchObject({ hasSecretAccessKey: false, hasAccessKeyId: true });
    expect(h.audit).toHaveBeenCalledTimes(3);
    expect(h.audit.mock.calls[1]![0].after).toEqual({
      operation: "update",
  ...
  it("authorizes draft credential reuse before probing and leaves saved values unchanged", async () => {
    ...
    for (const destinations of [local.backupDestinations, remote().backupDestinations]) {
      ...
      await expect(
        destinations.preflightDraft({ ...input, id: "destination-b" }),
      ).rejects.toMatchObject({ code: "NOT_FOUND" });
    }
    expect(h.destinationProbe).toHaveBeenCalledTimes(2);
    expect(h.destinationProbe.mock.calls[0]![1]).toMatchObject({
      bucket: stored.bucket,
      secretAccessKeyEnc: stored.secretAccessKeyEnc,
    });
  ...
  it("requires server authority before using a destination's saved server credentials", async () => {
    ...
    Object.assign(h.destinations.get("destination-a")!, {
      kind: "openship_server",
      serverId: "server-a",
    });
    const local = await native();
    for (const destinations of [local.backupDestinations, remote().backupDestinations]) {
      await expect(destinations.preflight("destination-a")).rejects.toMatchObject({
        code: "NOT_FOUND",
      });
      await expect(
        destinations.create({
    ...
    expect(await local.backupDestinations.preflight("destination-a")).toEqual(
      await remote().backupDestinations.preflight("destination-a"),
    );
    expect(h.destinationProbe).toHaveBeenCalledTimes(2);
    expect(h.destinationProbe.mock.calls[0]![1]).toMatchObject({
      serverId: "server-a",
      sftpPrivateKeyEnc: "enc1:stored-secret",
    });
    expect(h.audit).toHaveBeenCalledTimes(2);
  });
  ...
  it("refuses host key files in native preflight before reaching a provider", async () => {
    ...
    Object.assign(h.servers.get("server-a")!, {
      sshPrivateKey: null,
      sshKeyPath: "/tmp/host-private-key",
    });
  ...
  it("retains readonly denial and active-policy deletion refusal", async () => {
    ...
    for (const destinations of [local.backupDestinations, remote().backupDestinations]) {
      await expect(destinations.remove("destination-a")).rejects.toMatchObject({
        statusCode: 400,
        code: "BACKUP_DESTINATION_FAILED",
        message: "Destination has active policies",
      });
    }
    h.identity = { ...alice, credential: { organizationId: "org-a", readOnly: true } };
    for (const destinations of [local.backupDestinations, remote().backupDestinations]) {
      ...
      await expect(
        destinations.update("destination-a", { name: "Forbidden" }),
      ).rejects.toMatchObject({ code: "TOKEN_READ_ONLY" });
    }
    expect(h.destinationProbe).not.toHaveBeenCalled();
    expect(h.destinationUpdate).not.toHaveBeenCalled();
...
beforeEach(() => {
  ...
  h.backupPolicies.clear();
  h.backupRuns.clear();
  h.backupRestores.clear();
  ...
  h.tunnelUpsert.mockImplementation(async (data) => {
    const previous = [...h.tunnels.values()].find(
      (row) =>
        row.serverId === data.serverId &&
        row.remotePort === data.remotePort &&
        row.remoteHost === data.remoteHost,
    );
    const row = {
      ...previous,
      ...data,
      id: previous?.id ?? `tunnel-${data.serverId}-${data.remotePort}`,
    };
    h.tunnels.set(row.id, row);
    return row;
  });
  h.tunnelRemove.mockImplementation(async (id) => {
    h.tunnels.delete(id);
  });
  h.tunnelClose.mockResolvedValue(undefined);
  h.tunnelForward.mockResolvedValue({
    localPort: 49100,
  ...
  h.settingsDelete.mockImplementation(async () => {
    h.settings = undefined;
  });
  ...
  h.servers.set("server-a", {
    ...serverFixture(),
    organizationId: "org-a",
    sshPrivateKey: "enc1:stored-secret",
  });
  ...
  h.serverExec.mockResolvedValue({
  ...
  h.serverCreate.mockImplementation(async (data) => ({
    ...serverFixture("server-new"),
    ...data,
    organizationId: "org-a",
  }));
  h.serverUpdate.mockImplementation(async (id, patch) => ({ ...h.servers.get(id), ...patch }));
  h.componentCheck.mockImplementation(async (_executor, names: string[]) =>
    names.map((name) => ({
      name,
      label: name,
  ...
  h.containerUpsert.mockImplementation(async (row) => {
    ...
    h.containerRows.set(key, {
      ...serverContainerFixture(row.serverId, row.organizationId),
      ...h.containerRows.get(key),
      ...row,
    });
  });
...
describe("deployment inspection HTTP/native parity", () => {
  ...
  const cases = [
    {
      ...
      engine: h.deploymentContainerInfo,
      result: {
        containerId: "primary-service",
        status: "running",
        hostPortByContainerPort: { "8080": 49100 },
        usage,
      },
    },
  ...
  it("omits stored env snapshots from read-only HTTP and native responses", async () => {
    const stored = h.deployments.get("dep-project-a")!;
    stored.envVars = {
    ...
    stored.meta = {
      composeServices: [
        { name: "web", environment: { INTERNAL_VALUE: "plaintext-private-value" } },
      ],
    };

    const localResult = await (await native()).deployments.get("dep-project-a");
    const remoteResult = await remote().deployments.get("dep-project-a");
    for (const result of [localResult, remoteResult]) {
      expect(result).not.toHaveProperty("envVars");
      expect(result.meta.composeServices[0].environment).toEqual({ INTERNAL_VALUE: ENV_MASK });
      expect(JSON.stringify(result)).not.toContain("encrypted-private-value");
      expect(JSON.stringify(result)).not.toContain("plaintext-private-value");
    }
    expect(stored.envVars.NON_SECRET_LOOKING_KEY).toBe("encrypted-private-value");
    expect(stored.meta.composeServices[0].environment.INTERNAL_VALUE).toBe(
      "plaintext-private-value",
    );
  });

  it.each(cases)(
    "shares $method output and preserves the engine's tenant argument",
    async ({ method, engine, result }) => {
      ...
      expect(engine.mock.calls).toEqual([
        ["dep-project-a", "org-a"],
        ["dep-project-a", "org-a"],
      ]);
      expect(h.audit).not.toHaveBeenCalled();
  ...
  it.each(cases)(
    "checks current parent access before $method reaches the runtime",
    async ({ method, engine }) => {
      const local = await native();
      await expect(local.deployments[method]("dep-project-b")).rejects.toMatchObject({
        code: "NOT_FOUND",
      });
      await expect(remote().deployments[method]("dep-project-b")).rejects.toMatchObject({
        code: "NOT_FOUND",
      });
      h.members.set("org-a:alice", { id: "member-a", role: "restricted" });
      await expect(local.deployments[method]("dep-project-a")).rejects.toMatchObject({
        code: "NOT_FOUND",
      });
      await expect(remote().deployments[method]("dep-project-a")).rejects.toMatchObject({
        code: "NOT_FOUND",
      });
...
describe("hostname SSL HTTP/native parity", () => {
  ...
  beforeEach(() => {
    ...
    h.domains.set("domain-www", {
      ...h.domains.get("domain-a")!,
      id: "domain-www",
      projectId: "project-a",
  ...
  it("keeps primary and www renewal outcomes independent, with one audit per operation", async () => {
    ...
    expect(result).toMatchObject({
      ...
      status: "active",
      results: [
        { domain: "app.example.com", success: true },
        {
          domain: "www.app.example.com",
          success: false,
          status: "error",
          message: "www DNS is not pointed here",
        },
      ],
    });
    expect(h.manageSsl.mock.calls.map((call) => call[0])).toEqual([
      "app.example.com",
      "www.app.example.com",
      "app.example.com",
      "www.app.example.com",
  ...
  it("reports provisioning when a provider issues no certificate", async () => {
    ...
    for (const deployments of [(await native()).deployments, remote().deployments]) {
      expect(await deployments.renewSsl({ domain: "app.example.com" })).toMatchObject({
        success: false,
        status: "provisioning",
      });
    }
  ...
  it("rejects read-only credentials before renewal and rechecks each hostname's organization", async () => {
    h.identity = { ...alice, credential: { organizationId: "org-a", readOnly: true } };
    for (const deployments of [(await native()).deployments, remote().deployments]) {
      await expect(deployments.renewSsl({ domain: "app.example.com" })).rejects.toMatchObject({
        code: "TOKEN_READ_ONLY",
      });
    }
    expect(h.manageSsl).not.toHaveBeenCalled();
    ...
    const result = await (
      await native()
    ).deployments.renewSsl({ domain: "app.example.com", includeWww: true });
  ...
  it("preserves certificate errors and refuses native host execution before ACME", async () => {
    ...
    vi.stubEnv("OPENSHIP_NATIVE_ALLOW_HOST_EXECUTION", "false");
    await expect(
      (await native()).deployments.renewSsl({ domain: "app.example.com" }),
...
describe("project controls HTTP/native parity", () => {
  it("imports the scanner's Compose metadata through the shared create path and audits once", async () => {
    ...
    try {
      ...
      h.localInfo.mockResolvedValue(info);
      h.projectCreate.mockImplementation(async (input) => ({
        ...projectFixture("imported", input.name),
      ...
      const response = await app.request("/api/projects/import", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...command, services: [{ name: "untrusted-override" }] }),
      });
      ...
      for (const [event] of h.audit.mock.calls) {
        expect(event).toMatchObject({
          ...
          after: { source: "local", serviceCount: 1 },
        });
...
describe("domain and DNS HTTP/native parity", () => {
  ...
  it("lists and creates domains with a project grant and preserves sibling results", async () => {
    ...
    const result = {
      ...
      records: { mode: "external", records: [] },
      www: { id: "www", hostname: "www.example.com" },
    };
    ...
    expect(await remote().domains.create("project-a", input)).toEqual(
      await local.domains.create("project-a", input),
    );
  ...
  it("returns the same failed-verification result and audit while retaining HTTP 422", async () => {
    const result = {
      ...
      cnameVerified: false,
      txtVerified: true,
      message: "DNS is still propagating",
    };
    h.domainVerify.mockResolvedValue(result);
    const local = await native();
    expect(await remote().domains.verify("domain-a", { force: true })).toEqual(
      await local.domains.verify("domain-a", { force: true }),
    );
    expect(h.domainVerify).toHaveBeenCalledTimes(2);
    expect(h.domainVerify.mock.calls[0]![2]).toMatchObject({ force: true });
    expect(h.audit).toHaveBeenCalledTimes(2);
    expect(h.notify).toHaveBeenCalledTimes(2);
    expect((await app.request("/api/domains/domain-a/verify", { method: "POST" })).status).toBe(
      422,
    );
  });
  ...
  it("refuses foreign domains and explicit foreign DNS targets in both facades", async () => {
    ...
    for (const domains of [remote().domains, local.domains]) {
      ...
      await expect(domains.records("domain-a", { serverId: "server-b" })).rejects.toMatchObject({
        code: "NOT_FOUND",
      });
    }
  ...
  it("passes only the caller's scope and resource authorization into batch verification", async () => {
    h.domainPending.mockResolvedValue({
      verified: 0,
      stillPending: 0,
      failed: 0,
      total: 0,
    ...
    const local = await native();
    expect(await local.domains.verifyPending({ limit: 10 })).toEqual(
      await remote().domains.verifyPending({ limit: 10 }),
    );
    for (const [input, contextFor] of h.domainPending.mock.calls) {
      expect(input).toEqual({ limit: 10, organizationId: "org-a" });
      expect(await contextFor("domain-a", "verify")).toMatchObject({
        organizationId: "org-a",
        userId: "alice",
      });
  ...
  it("keeps certificate contents out of audit and preserves the cloud upload refusal", async () => {
    h.domainUpload.mockResolvedValue({
      domain: "app.example.com",
      sslStatus: "active",
      issuer: "manual",
    });
    ...
    expect(await local.domains.uploadCert("domain-a", input)).toEqual(
      await remote().domains.uploadCert("domain-a", input),
    );
    expect(h.audit).toHaveBeenCalledTimes(2);
    ...
    h.env.CLOUD_MODE = true;
    await expect(local.domains.uploadCert("domain-a", input)).rejects.toMatchObject({
      statusCode: 404,
    });
    await expect(remote().domains.uploadCert("domain-a", input)).rejects.toMatchObject({
      status: 404,
    });
  ...
  it("enforces administrator membership on both credential writes and never audits tokens", async () => {
    ...
    const input = {
      ...
      name: "Production",
      apiToken: "never-audit-dns-token",
    };
    const local = await native();
    ...
    for (const dns of [remote().dns, local.dns]) {
      ...
      await expect(dns.removeCredential("dns-a")).rejects.toMatchObject({
...
describe("project routing HTTP/native parity", () => {
  ...
  it("retains the sanitizer, project/domain checks, edge push, and one audit per facade", async () => {
    ...
    const input = {
      domainId: "domain-a",
      pathPrefix: " api ",
      spec: { access: { methods: ["get", "GET", "made-up"] } },
    };
    const local = await native();
    expect(await remote().projects.createRouteRule("project-a", input)).toEqual(
      await local.projects.createRouteRule("project-a", input),
    );
    for (const [data] of h.ruleCreate.mock.calls)
    ...
    for (const projects of [remote().projects, local.projects]) {
      await expect(
        projects.createRouteRule("project-a", { domainId: "domain-b" }),
  ...
  it("rejects forged rule/project pairs and keeps rule deletion scoped", async () => {
    ...
    for (const projects of [remote().projects, local.projects]) {
      await expect(
        projects.updateRouteRule("project-sibling", { ruleId: rule.id, enabled: false }),
      ).rejects.toMatchObject({ code: "NOT_FOUND" });
  ...
  it("connects a primary domain through the same domain operation and retains legacy errors", async () => {
    h.domainCreate.mockResolvedValue({
      domain: domainFixture(),
      records: { mode: "external", records: [] },
      wwwError: "sibling is already owned",
    });
    const local = await native();
    const input = { domain: " app.example.com ", externalIngress: true, includeWww: true };
    expect(await remote().projects.connectDomain("project-a", input)).toEqual(
      await local.projects.connectDomain("project-a", input),
    );
    ...
    for (const projects of [remote().projects, local.projects]) {
      await expect(projects.connectDomain("project-a", input)).rejects.toMatchObject({
        statusCode: 400,
        details: { success: false, message: "Domain already in use" },
      });
    }
  });
});
...
describe("provider credential HTTP/native parity", () => {
  ...
  it("shares verification, tenant arguments, masking and secret-free audit", async () => {
    ...
    const input = {
      ...
      selector: "ghcr.io",
      values: { username: "operator", secret: "do-not-audit-this" },
    };
    for (const credentials of [local.credentials, remote().credentials]) {
      ...
      expect(
        await credentials.update(credential.id, {
          name: "Rotated",
          values: { secret: "rotated-private-value" },
        }),
      ).toMatchObject({ name: "Rotated" });
  ...
  it("requires actual admin membership on every write even for a token with settings grants", async () => {
    h.identity = {
      ...alice,
      tokenScope: { tokenId: "credential-token" },
      credential: { organizationId: "org-a", readOnly: false },
    };
    ...
    for (const credentials of [local.credentials, remote().credentials]) {
      ...
      await expect(credentials.update("credential-a", { name: "Renamed" })).rejects.toMatchObject({
        code: "INSUFFICIENT_ROLE",
      });
      await expect(credentials.remove("credential-a")).rejects.toMatchObject({
        code: "INSUFFICIENT_ROLE",
      });
      await expect(credentials.verify("credential-a")).rejects.toMatchObject({
        code: "INSUFFICIENT_ROLE",
      });
    }
  ...
  it("uses the active organization for identifier reads and rejects malformed secret-bearing results", async () => {
    ...
    const local = await native();
    expect(await local.credentials.get("credential-a")).toEqual(
      await remote().credentials.get("credential-a"),
    );
    ...
    for (const credentials of [local.credentials, remote().credentials]) {
      await expect(credentials.get("credential-a")).rejects.toMatchObject({
...
describe("shared project transfers", () => {
  it("preserves partial promotion status and its structured result", async () => {
    h.transferCloud.mockResolvedValue({
      projectId: "project-a",
      imported: { project: 1 },
      localRemoved: false,
      unrecoverableSteps: 0,
    });
    const response = await app.request("/api/projects/project-a/transfer/to-cloud", {
      method: "POST",
    });
    expect(response.status).toBe(207);
  ...
  it("refuses unmapped fixed scopes before moving data", async () => {
    ...
    for (const projects of [local.projects, remote.projects]) {
      await expect(projects.transferToCloud("project-a")).rejects.toMatchObject({
        code: "CLOUD_SCOPE_UNAVAILABLE",
      });
      await expect(projects.transferToSelfHosted("project-a")).rejects.toMatchObject({
        code: "CLOUD_SCOPE_UNAVAILABLE",
      });
    }
...
describe("server installation HTTP/native parity", () => {
  ...
  it("accepts an exact server grant for body-derived diagnostic and installation targets", async () => {
    ...
    for (const servers of [local.servers, remote().servers]) {
      expect(await servers.check("server-a", { components: ["docker"] })).toMatchObject({
        ready: true,
        missing: [],
      });
      expect(
        await servers.installComponent("server-a", {
          component: "edge",
          config: { edgeImage: "untrusted:image" },
        }),
      ).toMatchObject({ success: true, component: "edge", logs: [] });
      expect(await servers.removeComponent("server-a", { component: "edge" })).toMatchObject({
        success: true,
      });
      await expect(servers.check("server-b")).rejects.toMatchObject({ code: "NOT_FOUND" });
      await expect(
        servers.installComponent("server-b", { component: "edge" }),
      ).rejects.toMatchObject({ code: "NOT_FOUND" });
    }
    expect(h.installEdge).toHaveBeenCalledTimes(2);
    for (const [, , config] of h.installEdge.mock.calls)
      expect(config.edgeImage).not.toBe("untrusted:image");
    expect(h.audit).toHaveBeenCalledTimes(6);
  });
  ...
  it("preserves missing-dependency recovery fields and does not deliver or install the edge", async () => {
    ...
    for (const servers of [local.servers, remote().servers])
      await expect(
        servers.installComponent("server-a", { component: "edge" }),
      ).rejects.toMatchObject({
        statusCode: 409,
        code: "missing_dependency",
        details: { component: "edge", missing: ["docker"], logs: [] },
      });
  ...
  it("shares install prompts, exact-grant responses, completion, and replay", async () => {
    ...
    for (const servers of [local.servers, remote().servers]) {
      let sessionId = "";
      const events = [];
      for await (const event of servers.installComponents("server-a", {
        components: ["edge"],
        ...
        if (event.event === "prompt")
          expect(await servers.respondToInstall({ sessionId, action: "override" })).toEqual({
            ok: true,
          });
      }
      expect(
        events.some(
          (event) => event.event === "complete" && JSON.parse(event.data).status === "completed",
        ),
      ).toBe(true);
      expect(await servers.getInstallSession({ sessionId })).toMatchObject({
        active: true,
        serverId: "server-a",
        status: "completed",
      });
      ...
      expect(
        replay.some(
          (event) =>
            event.event === "log" && JSON.parse(event.data).message === "decision:override",
        ),
      ).toBe(true);
  ...
  it("keeps session replay and response private, including busy-install errors", async () => {
    ...
    const local = await native();
    for (const servers of [local.servers, remote().servers]) {
      await expect(servers.getInstallSession({ sessionId: session.id })).rejects.toMatchObject({
        code: "NOT_FOUND",
      });
      await expect(
        servers.respondToInstall({ sessionId: session.id, action: "override" }),
      ).rejects.toMatchObject({ code: "NOT_FOUND" });
      await expect(
        servers.installEvents({ sessionId: session.id })[Symbol.asyncIterator]().next(),
      ).rejects.toMatchObject({ code: "NOT_FOUND" });
      const failure = await servers
        .installComponents("server-a", { components: ["edge"] })
  ...
  it("revalidates server grants before starting the next installation component", async () => {
    ...
    const operation = await getPlatformKernel().servers.openInstallStream(context, "server-a", {
      components: ["docker", "edge"],
    });
    const session = getActiveSetupSession()!;
    setupSessions.add(session.id);
    await drainServerInstallations();
    expect(getSetupSession(session.id)).toMatchObject({
      status: "failed",
      components: [
    ...
    expect(h.installEdge).not.toHaveBeenCalled();
    await expect(operation.data[Symbol.asyncIterator]().next()).rejects.toMatchObject({
      code: "NOT_FOUND",
    });
    await expect(local.servers.getInstallSession({ sessionId: session.id })).rejects.toMatchObject({
      code: "NOT_FOUND",
    });
  ...
  it("releases the retained monitor connection when either facade disconnects", async () => {
    ...
    for (const servers of [local.servers, remote().servers]) {
      ...
      const iterator = servers
        .monitor("server-a", { signal: abort.signal })
        [Symbol.asyncIterator]();
      const event = await iterator.next();
      expect(event.value).toMatchObject({
        event: "stats",
        data: JSON.stringify({ cpu: 10, memUsed: 1024 }),
      });
      abort.abort();
...
describe("managed container HTTP/native parity", () => {
  ...
  it("shares tenant-scoped cached fleet views, scans and issue classification", async () => {
    const local = await native();
    for (const servers of [local.servers, remote().servers]) {
      expect(await servers.listContainers("server-a")).toEqual([
        h.containerRows.get("server-a:edge"),
      ]);
      expect(await servers.listAllContainers()).toMatchObject([
        { server: { id: "server-a", projectCount: 2 } },
      ]);
      ...
      expect(await servers.scanContainers("server-a")).toMatchObject({
        ok: true,
        containers: [{ component: "edge", behind: true, down: false }],
      });
      ...
      await expect(
        servers.containerApplySession("server-b", { component: "edge" }),
      ).rejects.toMatchObject({ code: "NOT_FOUND" });
    }
  ...
  it("keeps reads separate from write authority and checks host policy before scans", async () => {
    ...
    for (const servers of [local.servers, remote().servers]) {
      ...
      await expect(
        servers.applyContainer("server-a", { component: "edge" })[Symbol.asyncIterator]().next(),
      ).rejects.toMatchObject({ code: "NOT_FOUND" });
    ...
    vi.stubEnv("OPENSHIP_NATIVE_ALLOW_HOST_EXECUTION", "false");
    for (const servers of [local.servers, remote().servers])
      await expect(servers.scanContainers("server-a")).rejects.toMatchObject({
  ...
  it("reuses an in-flight swap across apply and read-only subscribers, without cancelling it on disconnect", async () => {
    ...
    for (const servers of [local.servers, remote().servers]) {
      ...
      const started = servers
        .applyContainer("server-a", { component: "edge" })
        [Symbol.asyncIterator]();
      let sessionId = "";
      try {
        ...
        const duplicate = servers
          .applyContainer("server-a", { component: "edge" })
          [Symbol.asyncIterator]();
        const reader = servers
          .containerApplyEvents("server-a", { component: "edge" })
          [Symbol.asyncIterator]();
        try {
          expect((await duplicate.next()).value!.event).toBe("steps");
          expect((await reader.next()).value!.event).toBe("steps");
          expect(
            await servers.containerApplySession("server-a", { component: "edge" }),
          ).toMatchObject({ active: true, sessionId });
        ...
        await vi.waitFor(() =>
          expect(getContainerApplySession(sessionId)?.subscribers.size).toBe(0),
        );
      } finally {
        complete.resolve();
        await started.return?.();
      ...
      expect(await servers.containerApplySession("server-a", { component: "edge" })).toEqual({
        active: false,
      });
      await expect(
        servers
  ...
  it("preserves failed swap outcomes and refuses unprovisioned mail before opening events", async () => {
    h.reconcileEdge.mockResolvedValue({
      updated: false,
      edgeDown: false,
      error: "takeover consent required",
    });
    const local = await native();
    for (const servers of [local.servers, remote().servers]) {
      await expect(
        servers
          .applyContainer("server-a", { component: "mail", intent: "repair" })
          [Symbol.asyncIterator]()
          .next(),
      ).rejects.toMatchObject({ code: "MAIL_SERVER_NOT_PROVISIONED" });
      const events = [];
      for await (const event of servers.applyContainer("server-a", { component: "edge" }))
        events.push(event);
      expect(
        events.some(
          (event) => event.event === "complete" && JSON.parse(event.data).status === "failed",
        ),
      ).toBe(true);
  ...
  it("derives bulk targets from the current tenant and records their disposition once per call", async () => {
    ...
    for (const servers of [local.servers, remote().servers]) {
      ...
      expect(result).toEqual({
        started: [
          { serverId: "server-a", serverName: "Production", component: "edge", intent: "update" },
        ],
        skipped: [],
      });
      await drainBackgroundWork();
    ...
    expect(h.audit).toHaveBeenCalledTimes(2);
    for (const [record] of h.audit.mock.calls)
      expect(record.after.started).toEqual(["server-a:edge:update"]);
...
describe("server HTTP/native parity", () => {
  ...
  it("preserves encrypted credential writes, public presentation and one secret-free audit", async () => {
    ...
    const input = {
      ...
      sshKeyPassphrase: "passphrase-value",
    };
    const outputs = [];
    for (const servers of [local.servers, remote().servers])
      outputs.push(await servers.create(input));
  ...
  it("keeps list/detail counts, fixed tenant ownership, and runtime input validation", async () => {
    ...
    for (const servers of [local.servers, remote().servers]) {
      expect(await servers.list()).toEqual([
        expect.objectContaining({ id: "server-a", projectCount: 2 }),
      ]);
      expect(await servers.get("server-a")).toMatchObject({
        projectCount: 2,
        hasStoredKeyMaterial: true,
      });
      await expect(servers.get("server-b")).rejects.toMatchObject({ code: "NOT_FOUND" });
      await expect(
        servers.create({ sshHost: "example.com", sshPort: 99999 }),
      ).rejects.toMatchObject({ code: "VALIDATION_ERROR" });
    }
  ...
  it("requires server admin for execution and rate-limit writes, beyond a write grant", async () => {
    ...
    const local = await native();
    for (const servers of [local.servers, remote().servers]) {
      await expect(servers.update("server-a", { name: "Renamed" })).resolves.toMatchObject({
        name: "Renamed",
      });
      await expect(servers.exec("server-a", { command: "whoami" })).rejects.toMatchObject({
        code: "NOT_FOUND",
      });
      await expect(servers.updateRateLimit("server-a", { rps: 20 })).rejects.toMatchObject({
        code: "NOT_FOUND",
      });
  ...
  it("returns the same partial teardown result and keeps HTTP 409", async () => {
    h.serverWorkloads.mockResolvedValue([
      { id: "project-a", name: "Application", appTemplateId: null },
    ]);
    h.projectTeardown.mockResolvedValue({
      rowDeleted: false,
      orphaned: [],
      unrecoverable: [{ error: "volume busy" }],
    });
    const local = await native();
    ...
    const response = await app.request("/api/system/servers/server-a?destroyOnSource=true", {
      method: "DELETE",
    });
  ...
  it("preserves local-row refusal details and checks host policy before execution", async () => {
    ...
    const local = await native();
    for (const servers of [local.servers, remote().servers]) {
      await expect(servers.update("server-a", { sshUser: "deployer" })).rejects.toMatchObject({
        message: expect.stringContaining("display-only"),
      });
      await expect(servers.exec("server-a", { command: "whoami" })).rejects.toMatchObject({
        code: "HOST_EXECUTION_DISABLED",
      });
    }
    await expect(local.servers.update("server-a", { sshUser: "deployer" })).rejects.toMatchObject({
      details: { fields: ["sshUser"] },
    });
  ...
  it("retains live rate-limit sanitization and verification, and does not audit command output", async () => {
    const local = await native();
    for (const servers of [local.servers, remote().servers]) {
      await servers.updateRateLimit("server-a", {
        rps: 7.9,
        burst: -2,
        whitelist: ["10.0.0.0/8", "invalid; directive"],
      });
      expect(await servers.exec("server-a", { command: "  whoami  " })).toMatchObject({
        output: "private-output",
      });
    }
    for (const [config] of h.serverRateApply.mock.calls)
      expect(config).toEqual({ rps: 7, burst: 0, whitelist: ["10.0.0.0/8"] });
    expect(h.serverRateRead).toHaveBeenCalledTimes(4);
    expect(h.audit.mock.calls.filter(([event]) => event.eventType === "server.exec")).toHaveLength(
      2,
    );
  ...
  it("keeps failed module steps as results and uses explicit consent mode", async () => {
    ...
    const local = await native();
    for (const servers of [local.servers, remote().servers])
      expect(await servers.applyModule("server-a", { module: "openresty" })).toEqual(result);
    for (const [, module, mode] of h.serverModuleApply.mock.calls)
      expect([module, mode]).toEqual(["openresty", "all"]);
  });
});
...
describe("HTTP/native system parity", () => {
  ...
  it("shares deployment metadata and masked settings without requiring instance authority for reads", async () => {
    h.settings = {
      ...
      smtpPasswordEncrypted: "private-smtp-ciphertext",
      authMode: "local",
    };
    const local = await native();
    expect(await local.system.info()).toEqual(await remote().system.info());
    expect(await local.system.info()).toMatchObject({
      selfHosted: true,
  ...
  it("does not turn organization ownership or membership into instance administration", async () => {
    for (const role of ["owner", "member"]) {
      ...
      for (const system of [local.system, remote().system]) {
        await expect(system.updateSettings({ productMode: "mail" })).rejects.toMatchObject({
          statusCode: 403,
        });
        await expect(system.updateEmailSettings({ host: null })).rejects.toMatchObject({
          statusCode: 403,
        });
        ...
        await expect(
          system.removeUntrackedEdgeSite({ hostname: "orphan.example.test" }),
        ).rejects.toMatchObject({ statusCode: 403 });
      }
    }
  ...
  it("rechecks the persisted instance role for a previously created scope", async () => {
    ...
    const local = await native();
    for (const system of [local.system, remote().system])
      expect(await system.updateSettings({ productMode: "mail" })).toEqual({ ok: true });
    h.instanceRoles.delete("alice");
    for (const system of [local.system, remote().system])
      await expect(system.updateSettings({ productMode: "platform" })).rejects.toMatchObject({
        code: "FORBIDDEN",
      });
  ...
  it("allows instance reads with an unbound read-only credential while refusing mutations", async () => {
    ...
    const local = await native();
    for (const system of [local.system, remote().system]) {
      expect(await system.listUntrackedEdgeSites()).toEqual({
        scanned: true,
        orphans: [],
        knownCount: 2,
      });
  ...
  it("encrypts SMTP passwords, retains an omitted password, and audits only setting names", async () => {
    ...
    for (const system of [local.system, remote().system]) {
      expect(
        await system.updateEmailSettings({
      ...
      expect(await system.getEmailSettings()).toMatchObject({
        host: "smtp.example.test",
        port: 587,
        user: "admin",
        hasPassword: true,
      ...
      await system.updateSettings({
        tunnelToken: "secret-tunnel-token",
  ...
  it("retains pinned auth mode and deliberate zero-auth guards in both entry points", async () => {
    ...
    for (const system of [local.system, remote().system]) {
      ...
      await expect(system.updateSettings({ authMode: "cloud" })).rejects.toMatchObject({
        statusCode: 409,
      });
      h.env.OPENSHIP_AUTH_MODE = undefined;
      h.env.OPENSHIP_ALLOW_ZERO_AUTH = false;
      await expect(
        system.updateSettings({ authMode: "none", confirm: "I-understand-no-auth" }),
      ).rejects.toMatchObject({ statusCode: 403 });
      h.env.OPENSHIP_ALLOW_ZERO_AUTH = true;
      await expect(system.updateSettings({ authMode: "none" })).rejects.toMatchObject({
        statusCode: 400,
      });
      expect(
        await system.updateSettings({ authMode: "none", confirm: "I-understand-no-auth" }),
      ).toEqual({ ok: true });
      await expect(system.updateSettings({ productMode: "unknown-product" })).rejects.toMatchObject(
        { statusCode: 400 },
      );
    }
    expect(h.settingsUpsert).toHaveBeenCalledTimes(2);
    expect(
      h.audit.mock.calls.filter(([event]) => event.eventType === "auth-mode-changed"),
    ).toHaveLength(2);
  });
  ...
  it("keeps host-control writes restricted to the machine's founding organization", async () => {
    ...
    const local = await native();
    for (const system of [local.system, remote().system])
      await expect(system.updateSettings({ hostControl: false })).rejects.toMatchObject({
        statusCode: 403,
      });
    expect(h.settingsUpsert).not.toHaveBeenCalled();
    ...
    try {
      for (const system of [founding.system, remote("org_founder").system])
        expect(await system.updateSettings({ hostControl: false })).toEqual({ ok: true });
  ...
  it("returns test-mail delivery failure as a typed result while rejecting invalid recipients", async () => {
    ...
    const local = await native();
    for (const system of [local.system, remote().system]) {
      expect(await system.sendTestEmail({ to: " person@example.test " })).toEqual({
        ok: false,
        error: "SMTP connection refused",
      });
      await expect(system.sendTestEmail({ to: "invalid" })).rejects.toMatchObject({
        statusCode: 400,
        code: "INVALID_INSTANCE_SETTINGS",
      });
    }
  ...
  it("confines native directory browsing and excludes symlinks and hidden directories", async () => {
    ...
    try {
      ...
      const local = await native();
      await expect(local.system.browse()).rejects.toMatchObject({
      ...
      for (const system of [local.system, remote().system]) {
        await expect(system.browse({ path: directory })).rejects.toMatchObject({
          code: "SOURCE_PATH_NOT_ALLOWED",
        });
        await expect(system.browse({ path: join(root, "escape") })).rejects.toMatchObject({
          code: "SOURCE_PATH_NOT_ALLOWED",
        });
      }
    } finally {
...
describe("HTTP/native server tunnel parity", () => {
  ...
  it("retains configured ports and auto-start when quick-open omits those fields", async () => {
    ...
    const local = await native();
    for (const servers of [local.servers, remote().servers]) {
      const row = await servers.saveTunnel("server-a", {
        remotePort: "5432",
        localPort: 49100,
        autoStart: true,
      });
  ...
  it("shares one running forward and reports the assigned local port in both facades", async () => {
    ...
    expect(await remote().servers.stopTunnel("server-a", { tunnelId: row.id })).toMatchObject({
      running: false,
      url: null,
    });
  ...
  it("refuses foreign server and tunnel identifiers before touching a listener", async () => {
    ...
    for (const servers of [local.servers, remote().servers]) {
      ...
      await expect(
        servers.startTunnel("server-a", { tunnelId: "foreign-tunnel" }),
      ).rejects.toMatchObject({ code: "NOT_FOUND" });
      await expect(
        servers.removeTunnel("server-a", { tunnelId: "foreign-tunnel" }),
      ).rejects.toMatchObject({ code: "NOT_FOUND" });
    }
    expect(h.tunnelForward).not.toHaveBeenCalled();
  ...
  it("rechecks server grants before forwarding each connection", async () => {
    ...
    await expect(local.servers.startTunnel("server-a", { tunnelId: row.id })).rejects.toMatchObject(
      { code: "NOT_FOUND" },
    );
  });
  ...
  it("requires explicit native local-forwarding opt-in and preserves desktop-only HTTP availability", async () => {
    const local = await native();
    await expect(remote().servers.listTunnels("server-a")).rejects.toMatchObject({
    ...
    const row = await local.servers.saveTunnel("server-a", { remotePort: 5432 });
    await expect(local.servers.startTunnel("server-a", { tunnelId: row.id })).rejects.toMatchObject(
      { code: "LOCAL_FORWARDING_DISABLED" },
    );
    expect(h.tunnelForward).not.toHaveBeenCalled();
    vi.stubEnv("OPENSHIP_NATIVE_ALLOW_LOCAL_FORWARDING", "true");
    expect(await local.servers.startTunnel("server-a", { tunnelId: row.id })).toMatchObject({
      running: true,
    });
  });
});
...
vi.mock("@repo/platform/engine/modules/backups/backup.orchestrator", () => ({
  backupOrchestrator: { enqueue: h.backupEnqueue },
}));
vi.mock("@repo/platform/engine/modules/backups/restore.orchestrator", () => ({
  restoreOrchestrator: {
    beginPrepare: h.backupPrepare,
    apply: h.backupApply,
    cancel: h.backupCancel,
  },
}));
vi.mock("@repo/platform/engine/modules/backups/triggers/cron", async (importOriginal) => ({
  ...
  syncPolicySchedule: h.backupSyncSchedule,
  removePolicySchedule: h.backupRemoveSchedule,
}));
...
describe("backup HTTP/native parity with retained policy services", () => {
  ...
  beforeEach(() => {
    for (const [id, organizationId] of [
      ...
      h.destinations.set(id!, {
        id: id!,
        organizationId: organizationId!,
      } as StoredBackupDestination);
    }
    h.backupPolicies.set("policy-a", backupPolicyFixture());
    h.backupRuns.set("run-a", {
      ...backupRunFixture(),
      artifacts: [
        {
          name: "archive",
          metadata: { restoreCommand: "restore --password=backup-secret", format: "tar" },
        },
      ],
    });
    h.backupRestores.set("restore-a", {
      ...backupRestoreFixture(),
      confirmationToken: "in-force-token",
    });
    h.backupPolicyCreate.mockImplementation(async (input) => {
      const row = {
        ...backupPolicyFixture(input.id, input.projectId, input.destinationId),
        ...input,
      };
      h.backupPolicies.set(row.id, row);
      return row;
    });
    h.backupPolicyUpdate.mockImplementation(async (id, input) => {
      const row = { ...h.backupPolicies.get(id)!, ...input };
      h.backupPolicies.set(id, row);
      return row;
    });
    h.backupPolicyDelete.mockImplementation(async (id) => {
      h.backupPolicies.delete(id);
    });
    h.backupRetentionLock.mockImplementation(async (id, until) => {
      h.backupRuns.get(id)!.retentionLockedUntil = until?.toISOString() ?? null;
    });
    h.backupEnqueue.mockResolvedValue({ runId: "run-a", runIds: ["run-a"] });
    h.backupPrepare.mockResolvedValue({
      restoreId: "restore-a",
      confirmationToken: "in-force-token",
    });
    h.backupApply.mockResolvedValue(undefined);
    h.backupCancel.mockResolvedValue({
      accepted: true,
      status: "applying",
      destructive: true,
      forced: false,
  ...
  it("reuses policy validation, retention defaults, schedule refresh and webhook rotation", async () => {
    const local = await native();
    for (const ops of [local.backups, remote().backups]) {
      const created = await ops.createPolicy("project-a", {
      ...
      const updated = await ops.updatePolicy(created.id, {
      ...
      await expect(
        ops.updatePolicy(created.id, { cronExpression: "not a cron" }),
      ).rejects.toMatchObject({ code: "BACKUP_OPERATION_FAILED" });
  ...
  it("confines policy source and destination references before creating anything", async () => {
    ...
    for (const ops of [local.backups, remote().backups]) {
      await expect(
        ops.createPolicy("project-a", { destinationId: "destination-b" }),
      ).rejects.toMatchObject({ code: "NOT_FOUND" });
      await expect(
        ops.createPolicy("project-a", { destinationId: "destination-a", serviceId: "service-b" }),
      ).rejects.toMatchObject({ code: "NOT_FOUND" });
    }
  ...
  it("requires both destination and project authority before running or restoring", async () => {
    ...
    for (const ops of [local.backups, remote().backups]) {
      ...
      await expect(
        ops.applyRestore("restore-a", { confirmationToken: "in-force-token" }),
      ).rejects.toMatchObject({ code: "NOT_FOUND" });
    }
  ...
  it("returns the orchestrator's reused token and preserves cooperative cancellation details", async () => {
    const local = await native();
    for (const ops of [local.backups, remote().backups]) {
      expect(await ops.prepareRestore("run-a")).toEqual({
        restoreId: "restore-a",
        confirmationToken: "in-force-token",
      });
      expect(await ops.applyRestore("restore-a", { confirmationToken: "in-force-token" })).toEqual({
        ok: true,
      });
      expect(await ops.cancelRestore("restore-a")).toEqual({
        ok: true,
        accepted: true,
        status: "applying",
        destructive: true,
  ...
  it("preserves retention locking and clearing and rejects invalid timestamps before writes", async () => {
    const local = await native();
    for (const ops of [local.backups, remote().backups]) {
      expect(await ops.protectRun("run-a")).toEqual({
        ok: true,
        retentionLockedUntil: "2099-12-31T23:59:59.000Z",
      });
      expect(await ops.protectRun("run-a", { protected: false })).toEqual({
        ok: true,
        retentionLockedUntil: null,
      });
      await expect(ops.protectRun("run-a", { until: "invalid" })).rejects.toMatchObject({
